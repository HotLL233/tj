...
/**
 * StatsCards — uiverse.io style summary cards.
 * Gradient border top, icon on left, large colored value.
 * Hover glow + lift effect.
 */
const StatsCards: React.FC<StatsCardsProps> = ({ summary, onCardClick }) => {
  return (
    <Box
      sx={{
        display: 'grid',
    ...
    >
      {CARD_ITEMS.map(({ key, label, color, gradient, icon, tab }) => (
        <Paper
          key={key}
          elevation={0}
