...
const F_LIST = ['🔬 实验室送样与工作量双入口','📊 按周/月/日多维度统计','🔬 液相/气相仪器分类统计','📦 送样记录与专属统计','📋 汇总模板格式导出','👤 用户日志与纠错编辑','🔧 分组/项目管理','📝 审计日志 / 🗑️ 回收站'];
...
const Layout: React.FC = () => {
  ...
  return (<Box sx={{ display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
    ...
    <Dialog open={aboutOpen} onClose={() => setAboutOpen(false)} maxWidth="xs" fullWidth PaperProps={{ sx: { borderRadius: '2px' } }}>
      ...
      <DialogContent sx={{ textAlign: 'center' }}><Typography variant="subtitle2" color="primary" sx={{ mb: 2, fontWeight: 600 }}>v{serverVer || '...'}</Typography>
        <Box sx={{ textAlign: 'left', px: 2 }}>{F_LIST.map((text, i) => <Typography key={i} variant="body2" sx={{ py: 0.5 }}>{text}</Typography>)}</Box>
        <Typography variant="caption" sx={{ mt: 3, display: 'block', color: 'text.disabled' }}>&copy; 2026 HotLL</Typography>
      </DialogContent>
    </Dialog>
  </Box>);
};
