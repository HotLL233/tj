...
fn write_tree_sheet(
    ...
    // ─── Step 1: Write data FIRST (before any merge, to avoid cell corruption) ───
    let mut ri = HR + 1;
    ...
    // ─── Step 2: Merge G/H/I columns with empty shell ───
    ...
    // ─── Step 3: Write formulas to G/H/I top-left cells ───
    ...
    // ─── Step 4: Merge B(l1) + C(l2) with actual values (preserves data) ───
    let mut i = 0usize;
    while i < rows.len() {
        let ref_l1 = &rows[i].0;
    ...
    for &(gs, ge, _, _) in &groups {
        if ge > gs {
            // Read the l2 value from the already-written cell
            // (we can't easily read back, so extract from rows array by index)
            let row_idx = gs as usize - (HR + 1) as usize;
            if row_idx < rows.len() {
                let l2_val = &rows[row_idx].1;
                ws.merge_range(gs, CC, ge, CC, l2_val.as_str(), &fmt.fd)?;
            }
        }
    }
    ...
    for col in [CF, CG, CH, CI] {
        // Full range with column letter on both sides: e.g. =SUM(F3:F10)
        let cl = _cl(col);
        let formula = format!("=SUM({}{}:{}{})", cl, HR + 1, cl, tr - 1);
        ws.write_formula(tr, col, formula.as_str())?;
    }
