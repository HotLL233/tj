// ─── Export data aggregation helpers ────────────────────────────
// Tests: cargo test -- export_data

use std::collections::HashMap;
use rusqlite::Connection;
use crate::error::Result;

/// Parse "HYLY-LC-01(230106)" → (method_base, instrument_code, "液相"|"气相"|"其他")
...
/// Extract project code prefix: "HYLY-LC-01" → "HYLY"
pub fn extract_code(n: &str) -> &str { n.split('-').next().unwrap_or(n) }

/// Map short instrument name to full method name
...
/// Calculate month bounds from a YYYY-MM-DD reference date
...
/// Calculate week ranges covering a month (Mon-Sun)
pub fn week_ranges(start_ymd: &str) -> Vec<(String, String, String)> {
    ...
    while cur <= last_day {
        ...
        let label = format!("{:02}.{:02}-{:02}.{:02}", cur.month(), cur.day(), we.month(), we.day());
...
pub struct ProjectRows { pub lc: Vec<(String, String, i64)>, pub gc: Vec<(String, String, i64)> }

pub type FlatRow = (String, String, String, String, i64, bool); // (l1, l2, ic, ml, qty, is_gc)

/// Build lab→code→instrument hierarchy from DB records
pub fn build_lab_hierarchy(
    conn: &Connection, group_id: Option<i64>, d_start: &str, d_end: &str,
...
/// Flatten lab hierarchy to FlatRow list
...
mod tests {
    ...
    #[test]
    fn test_parse_instrument_lc() {
        let (_, code, itype) = parse_instrument("HYLY-LC-01(230106)");
        assert_eq!(code, "LC-01");
        assert_eq!(itype, "液相");
    }

    #[test]
    fn test_parse_instrument_gc() {
        let (_, code, itype) = parse_instrument("E003-GC-02");
        assert_eq!(code, "GC-02");
        assert_eq!(itype, "气相");
    }

    #[test]
    fn test_parse_instrument_other() {
        let (_, code, itype) = parse_instrument("SomeProject");
        assert_eq!(code, "");
        assert_eq!(itype, "其他");
    }
    ...
    fn test_extract_code() {
        assert_eq!(extract_code("HYLY-LC-01"), "HYLY");
        assert_eq!(extract_code("E003-GC-02"), "E003");
        assert_eq!(extract_code("Single"), "Single");
    }

    #[test]
    fn test_month_bounds() {
        let (s, e) = month_bounds("2026-06-15");
    ...
    fn test_month_bounds_december() {
        let (s, e) = month_bounds("2026-12-10");
    ...
    #[test]
    fn test_week_ranges() {
        let weeks = week_ranges("2026-06-01");
        assert!(!weeks.is_empty());
        // June 2026 starts on a Monday, first week should cover 6/1-6/7 or similar
        assert!(weeks[0].0.contains("06"));
    }

    #[test]
    fn test_method_full_name_known() {
        let name = get_method_full_name("410", "HYLY-LC-01(230106)");
        assert_eq!(name, "HYLY-230106-1-低温8℃-DAD");
    }

    #[test]
    fn test_method_full_name_unknown() {
        let name = get_method_full_name("410", "Unknown-XX-99");
        assert_eq!(name, "XX-99"); // falls back to second part after dash
    }
}
