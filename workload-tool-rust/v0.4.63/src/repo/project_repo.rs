...
const PROJ_SQL: &str =
    "SELECT p.id, p.name, COALESCE(p.notes,''), \
     ...
     p.created_at, \
     p.high_item_id, hi.name AS high_item_name \
     FROM projects p \
     ...
     LEFT JOIN high_items hi ON p.high_item_id = hi.id";
...
fn row_to_project(row: &rusqlite::Row) -> rusqlite::Result<ProjectResponse> {
    ...
    Ok(ProjectResponse {
        ...
        high_item_id: row.get::<_, Option<i64>>(11).unwrap_or(None),
        high_item_name: row.get::<_, Option<String>>(12).unwrap_or(None),
    })
}
...
pub fn create(pool: &DbPool, body: &ProjectCreate) -> Result<ProjectResponse> {
    ...
    conn.execute(
        "INSERT INTO projects (group_id, name, full_name, notes, sort_order, is_active, high_item_id) VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7)",
        rusqlite::params![group_id, body.name, fnm, nt, so, ia, body.high_item_id],
    )?;
...
pub fn update(pool: &DbPool, id: i64, body: &ProjectUpdate) -> Result<ProjectResponse> {
    ...
    if let Some(hi) = body.high_item_id {
        conn.execute("UPDATE projects SET high_item_id=?1 WHERE id=?2", (hi, id))?;
    }
    // Replace lab links
    if let Some(ref lab_ids) = body.lab_ids {
