...
const ManagePage: React.FC = () => {
  ...
  // v0.4.63: 高项
  const [highItems, setHighItems] = useState<any[]>([]);
  const loadHighItems = useCallback(async () => { try { const r = await getHighItems(); if (r.code === 0 && r.data) setHighItems(r.data); } catch {} }, []);
  // v0.4.63: 导入用户对话框
  ...
  useEffect(() => {
    if (tb === 'sampleinfo') { loadSiTypes(); loadSiRecords(); loadSiStats(); loadSiColumns(); loadHighItems(); }
  ...
  return (
      ...
      <DialogContent>
        {projectEditItem && <>
          ...
          <FormControl fullWidth size="small" sx={{ mt: 2 }}>
            <InputLabel>关联高项</InputLabel>
            <Select value={projectEditItem.high_item_id || ''} label="关联高项" onChange={e => setProjectEditItem({ ...projectEditItem, high_item_id: e.target.value ? Number(e.target.value) : null })} sx={{ borderRadius: R }}>
              <MenuItem value="">无关联</MenuItem>
              {highItems.map(hi => <MenuItem key={hi.id} value={hi.id}>{hi.name}</MenuItem>)}
            </Select>
          </FormControl>
    ...
    <Dialog open={importDialogOpen} onClose={() => setImportDialogOpen(false)} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: R } }}>
      ...
      <DialogContent>
        <Box sx={{ mb: 2 }}>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>选择 CSV 格式的文件，第一行为表头：用户名,密码,真实姓名,所属部门,所属实验室,角色,启用</Typography>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>密码默认 123456，跳过已有用户名的行</Typography>
