    ...
    <Dialog open={aboutOpen} onClose={() => setAboutOpen(false)} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: '2px' } }}>
      ...
      <DialogContent>
        ...
        <Alert severity="info" sx={{ mb: 2, borderRadius: '2px' }}>
          <Typography variant="subtitle2" sx={{ fontWeight: 600, mb: 0.5 }}>v0.4.97 手机录入与倍率修复版</Typography>
          <Typography variant="body2">手机端样品登记短字段改为两列，长字段和附件保持整行。</Typography>
          <Typography variant="body2">手机端辅助操作按钮等宽排列，提交按钮独占整行。</Typography>
          <Typography variant="body2">分析检测的检测人原始记录恢复单价倍率显示和直接编辑。</Typography>
          <Typography variant="body2">人员汇总和人员图表仍按原始工作量统计，不应用单价倍率。</Typography>
        </Alert>
