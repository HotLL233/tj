...
const ManagePage: React.FC = () => {
  ...
  const defaultTC: { key: TV; label: string; perm?: string; icon: React.ReactNode; desc: string }[] = [
    ...
    { key: 'master-import', label: '主数据一键导入', perm: 'manage:settings', icon: <CloudUploadIcon />, desc: '按模板批量导入主数据及关联关系' },
  ...
  const loadTrash = async () => {
    try { const r = await getRecords({ include_deleted: true }); if (r.code === 0 && r.data) setTr(r.data.items.filter((x: any) => x.deleted_at != null)); } catch {};
  ...
  return (
            ...
            <Box key={m.id} sx={{ display: 'grid', gridTemplateColumns: { xs: 'minmax(0,1fr) auto', md: 'minmax(260px,1.6fr) minmax(180px,1fr) minmax(180px,1fr) auto' }, alignItems: 'center', gap: 1.5, px: { xs: 1.25, md: 2 }, py: 1.1, borderBottom: index === filtered.length - 1 ? 'none' : '1px solid #edf0f2' }}>
              <Box sx={{ minWidth: 0 }}>
                <Box sx={{ display: 'flex', gap: 0.75, alignItems: 'center', flexWrap: 'wrap' }}>
                  ...
                  {extractInstrumentFromMethodName(m.name) && <Chip label={`仪器: ${extractInstrumentFromMethodName(m.name)}`} size="small" color="info" sx={{ height: 22, borderRadius: R, fontSize: '0.68rem' }} />}
                </Box>
    ...
    {showSectionContent && tb === 'trash' && <Box>
      ...
      <Box sx={{ display: 'flex', gap: 1, mb: 2, flexWrap: 'wrap' }}>
        ...
        <Chip label={`检测类型 (${trTypes.length})`} size="small" onClick={() => setTrashTab('types')} color={trashTab === 'types' ? 'primary' : 'default'} variant={trashTab === 'types' ? 'filled' : 'outlined'} sx={{ borderRadius: R }} />
        <Chip label={`研发项目 (${trProjects.length})`} size="small" onClick={() => setTrashTab('projects')} color={trashTab === 'projects' ? 'primary' : 'default'} variant={trashTab === 'projects' ? 'filled' : 'outlined'} sx={{ borderRadius: R }} />
        <Chip label={`用户 (${trUsers.length})`} size="small" onClick={() => setTrashTab('users')} color={trashTab === 'users' ? 'primary' : 'default'} variant={trashTab === 'users' ? 'filled' : 'outlined'} sx={{ borderRadius: R }} />
      </Box>
            ...
            <TableBody>{tr.map(r => (
              <TableRow key={r.id} hover>
                ...
                <TableCell align="right" sx={adaptiveCellSx(trashRecordWidths.actions)}>
                  <Button size="small" color="success" onClick={async () => { try { const res = await restoreRecord(r.id); if (res.code === 0) { sm('恢复成功'); loadTrash(); } else sm(res.message, true); } catch { sm('恢复失败', true); } }} sx={{ borderRadius: R }}>恢复</Button>
                </TableCell>
              </TableRow>
            ))}</TableBody>
          </Table>
