...
fn operator(headers: &HeaderMap) -> String {
    headers.get("authorization").and_then(|value| value.to_str().ok())
        ...
        .map(|claims| claims.username).unwrap_or_else(|| "system".into())
}
...
async fn create(State(pool): State<DbPool>, headers: HeaderMap, Json(body): Json<RecordCreate>) -> Result<Json<ApiResponse<RecordResponse>>> {
    ...
    Ok(Json(ApiResponse::ok(record_service::create_record(&pool, &record, &operator(&headers))?)))
}

async fn update(State(pool): State<DbPool>, headers: HeaderMap, Path(id): Path<i64>, Json(body): Json<RecordUpdate>) -> Result<Json<ApiResponse<RecordResponse>>> {
    // Service layer: validates not-deleted, change detection
    Ok(Json(ApiResponse::ok(record_service::update_record(&pool, id, &body, &operator(&headers))?)))
}
...
async fn soft_delete(State(pool): State<DbPool>, headers: HeaderMap, Path(id): Path<i64>) -> Result<Json<ApiResponse<()>>> {
    ...
    record_service::delete_record(&pool, id, &operator(&headers))?;
    Ok(Json(ApiResponse::ok_msg("删除成功")))
}

async fn restore(State(pool): State<DbPool>, headers: HeaderMap, Path(id): Path<i64>) -> Result<Json<ApiResponse<RecordResponse>>> {
    Ok(Json(ApiResponse::ok(record_repo::restore(&pool, id, &operator(&headers))?)))
}
...
async fn delete_by_user(
    ...
    let count = record_repo::delete_by_user(&pool, &user_name, q.start.as_deref(), q.end.as_deref(), &operator(&headers))?;
