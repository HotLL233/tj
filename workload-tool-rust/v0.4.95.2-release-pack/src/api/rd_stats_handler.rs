use axum::{extract::{Query, State}, http::HeaderMap, Json, Router, routing::get};
...
fn scope_user_name(headers: &HeaderMap) -> Option<String> {
    let claims = extract_claims(headers)?;
    if claims.is_admin || claims.permissions.iter().any(|p| p == "*" || p == "stats:workload:view-all") {
        None
    } else {
...
fn build_where(start: Option<&str>, end: Option<&str>, user_name: Option<&str>) -> (String, Vec<String>) {
    ...
    if let Some(user) = user_name { let i = params.len()+1; clauses.push(format!("wr.user_name=?{}", i)); params.push(user.to_string()); }
...
mod tests {
    ...
    fn rd_score_uses_record_snapshot() {
        assert_eq!(coeff_sql(), "COALESCE(SUM(wr.quantity * wr.coefficient_snapshot), 0.0)");
    }
}
...
async fn summary(State(pool): State<DbPool>, headers: HeaderMap, Query(q): Query<StatsQuery>) -> Result<Json<ApiResponse<StatsSummary>>> {
    let scoped_user = scope_user_name(&headers);
    let (wc, params) = build_where(q.start.as_deref(), q.end.as_deref(), scoped_user.as_deref());
    let conn = pool.get()?;
...
async fn by_user(State(pool): State<DbPool>, headers: HeaderMap, Query(q): Query<StatsQuery>) -> Result<Json<ApiResponse<Vec<UserStats>>>> {
    let scoped_user = scope_user_name(&headers);
    let (wc, params) = build_where(q.start.as_deref(), q.end.as_deref(), scoped_user.as_deref());
    let conn = pool.get()?;
...
async fn by_project(State(pool): State<DbPool>, headers: HeaderMap, Query(q): Query<StatsQuery>) -> Result<Json<ApiResponse<Vec<ProjectStats>>>> {
    let scoped_user = scope_user_name(&headers);
    let (wc, params) = build_where(q.start.as_deref(), q.end.as_deref(), scoped_user.as_deref());
    let conn = pool.get()?;
    let mut param_refs: Vec<&dyn rusqlite::types::ToSql> = params.iter().map(|s| s as &dyn rusqlite::types::ToSql).collect();
...
async fn by_type(State(pool): State<DbPool>, headers: HeaderMap, Query(q): Query<StatsQuery>) -> Result<Json<ApiResponse<Vec<TypeStats>>>> {
    let scoped_user = scope_user_name(&headers);
    let (wc, params) = build_where(q.start.as_deref(), q.end.as_deref(), scoped_user.as_deref());
    let conn = pool.get()?;
    let param_refs: Vec<&dyn rusqlite::types::ToSql> = params.iter().map(|s| s as &dyn rusqlite::types::ToSql).collect();
...
async fn by_instrument(State(pool): State<DbPool>, headers: HeaderMap, Query(q): Query<StatsQuery>) -> Result<Json<ApiResponse<Vec<InstrumentStats>>>> {
    let scoped_user = scope_user_name(&headers);
    let (wc, params) = build_where(q.start.as_deref(), q.end.as_deref(), scoped_user.as_deref());
    let conn = pool.get()?;
    let param_refs: Vec<&dyn rusqlite::types::ToSql> = params.iter().map(|s| s as &dyn rusqlite::types::ToSql).collect();
...
async fn by_division(
    ...
    let scoped_user = scope_user_name(&headers);
    let (wc, mut params) = build_where(q.start.as_deref(), q.end.as_deref(), scoped_user.as_deref());
    let conn = pool.get()?;
