...
pub fn list(pool: &DbPool, group_id: Option<i64>, active_only: bool, method_type: Option<&str>) -> Result<Vec<ProjectResponse>> {
    ...
    sql.push_str(" ORDER BY CASE p.method_type WHEN '液相' THEN 0 WHEN '气相' THEN 1 WHEN '理化' THEN 2 WHEN 'ICP' THEN 3 WHEN '热分析' THEN 4 WHEN '检测类型' THEN 5 ELSE 6 END, pg.sort_order, p.sort_order");
...
// v0.2.9: 三级树状批量导入
// 实验室(group) -> 研发项目(project, 作为 group 下) -> 检测方法(每个方法是独立 project, 隶属于研发项目)
// 实现策略: 研发项目本身存储为 project(parent_id=NULL, group_id=lab), 方法存储为 project(parent_id=research_proj_id, group_id=lab)
pub fn batch_import_tree(conn: &Connection, items: &[MethodImportItem]) -> Result<ImportSummary> {
    ...
    // 在 projects 表中新增 parent_id 列(用于二级关联)
    ...
    for ((lab_name, proj_name), methods) in &by_proj {
        ...
        // 3. upsert 研发项目(parent_id=0, group_id=gid)
        let existing_proj: Option<i64> = conn.query_row(
            "SELECT id FROM projects WHERE name=?1 AND group_id=?2 AND parent_id=0",
            rusqlite::params![proj_name, gid], |r| r.get(0)).ok();
        let pid = if let Some(id) = existing_proj {
            id
        } else {
        ...
        // 4. upsert 检测方法(parent_id=pid, group_id=gid, method_type=推断)
