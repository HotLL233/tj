...
async fn export_excel(
    ...
    let result: std::result::Result<Vec<u8>, String> = (|| {
        ...
        match export_data::query_sheet5_data(&conn, start, end) {
            Ok(data) => {
                ...
                let ws = wb.add_worksheet();
                export_write::write_sheet5(ws, &data, &fmt).map_err(|e| format!("Sheet5: {}", e))?;
        ...
        match export_data::query_sheet6_data(&conn, start, end) {
            Ok(data) => {
                ...
                let ws = wb.add_worksheet();
                export_write::write_sheet6(ws, &data, &fmt).map_err(|e| format!("Sheet6: {}", e))?;
    ...
    match result {
            Ok(data) => {
            // 审计日志：记录导出操作
            let desc = format!("导出Excel: {}-{}", start, end);
