...
import { getHelpDocuments, getHelpDocumentPageUrl, getHelpDocumentFileUrl } from '../api/client';
...
const HelpPage: React.FC = () => {
  ...
  const [viewing, setViewing] = useState<HelpDocument | null>(null);
  ...
  const handleOpen = (doc: HelpDocument) => {
    setViewing(doc);
    setPageNum(1);
  };
  ...
  const totalPages = viewing?.page_count ?? 1;
          ...
          {viewing && viewing.page_count > 0 ? (
            <>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, py: 1.5 }}>
                ...
                <IconButton size="small" disabled={pageNum >= totalPages} onClick={() => setPageNum(p => p + 1)}>
            ...
            <Box sx={{ p: 4, textAlign: 'center', width: '100%', bgcolor: '#fff' }}>
              <Typography color="text.secondary">
                不支持在线预览（{viewing.file_type.toUpperCase()}）
              </Typography>
              <Button
                variant="outlined"
                href={getHelpDocumentFileUrl(viewing.id)}
                target="_blank"
