...
pub fn update(pool: &DbPool, id: i64, body: &RecordUpdate, user_name: &str) -> Result<RdRecordResponse> {
    ...
    if let Some(m) = body.multiplier {
        changes.push(format!("multiplier -> {:.2}", m));
        let rows = tx.execute("UPDATE rd_work_records SET multiplier=?1 WHERE id=?2", (m, id))?;
        if rows == 0 {
            return Err(crate::error::AppError::NotFound("record not found".into()));
        }
        updated = true;
    }
    ...
    if let Some(gid) = body.group_id {
        if Some(gid) != existing.group_id { changes.push(format!("实验室ID {:?} → {}", existing.group_id, gid)); }
        let rows = tx.execute("UPDATE rd_work_records SET group_id=?1 WHERE id=?2", (gid, id))?;
        if rows == 0 {
            return Err(crate::error::AppError::NotFound("记录不存在".into()));
        }
        updated = true;
    ...
    if let Some(did) = body.division_id {
        if Some(did) != existing.division_id { changes.push(format!("部门ID {:?} → {}", existing.division_id, did)); }
        let rows = tx.execute("UPDATE rd_work_records SET division_id=?1 WHERE id=?2", (did, id))?;
        if rows == 0 {
            return Err(crate::error::AppError::NotFound("记录不存在".into()));
        }
        updated = true;
    ...
    if let Some(ref hi) = body.high_item {
        let hi_val = if hi.trim().is_empty() { None } else { Some(hi.trim()) };
        changes.push(format!("high_item -> {:?}", hi));
        let rows = tx.execute("UPDATE rd_work_records SET high_item=?1 WHERE id=?2", rusqlite::params![hi_val, id])?;
        if rows == 0 {
            return Err(crate::error::AppError::NotFound("record not found".into()));
        }
        updated = true;
