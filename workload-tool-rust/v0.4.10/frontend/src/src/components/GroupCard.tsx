...
const GroupCard: React.FC<GroupCardProps> = ({ group, onClick, themeColor }) => { 
  ...
  {!isMobile && <Box sx={{ display: 'flex', justifyContent: 'center', gap: 0.5 }}>
    <Chip label={`${group.project_count} 个项目`} size="small" variant="outlined" sx={{ borderRadius: '2px', borderColor: `${color}40`, color, fontWeight: 500 }} />
    {group.rd_record_count && group.rd_record_count > 0 && <Chip label={`${group.rd_record_count} 条送样`} size="small" variant="outlined" sx={{ borderRadius: '2px', borderColor: `${color}40`, color, fontWeight: 500 }} />}
  </Box>}
  </Paper>); };
