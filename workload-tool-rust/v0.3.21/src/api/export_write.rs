...
pub fn write_sheet6(
    ...
    for row in rows {
        ...
        // 个人汇总：根据本行类型构建公式，未写列用 0 代替，避免引用空单元格导致 #VALUE!
    ...
    // 合并用户列和 K 列（个人汇总）
    // v0.3.21 修复：merge_range 后单独对左上角单元格 write_formula，避免公式被覆盖
    for (start, end, user) in &user_groups {
        // 硬保护：start == end 时不 merge（Excel 不允许合并单个单元格）
        if *start == *end { continue; }
        ws.merge_range(*start, CA, *end, CA, user.as_str(), &fmt.fd)?;
        // K 列：先 merge，再对左上角单元格写入 SUM 公式
        ws.merge_range(*start, CK, *end, CK, "", &fmt.fd)?;
        let sum_formula = format!("=SUM({}{}:{}{})", col_letter(CK), start + 1, col_letter(CK), end + 1);
        ws.write_formula(*start, CK, sum_formula.as_str())?;
    }
