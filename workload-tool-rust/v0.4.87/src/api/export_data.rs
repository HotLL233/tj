...
/// 扁平行数据（Sheet 1 使用）
pub type FlatRow = (String, String, String, String, f64, i64, bool, f64, Option<String>);
...
fn add_user_filter(
    ...
    if let Some(user) = user_name {
        ...
        sql.push_str(&format!(" AND wr.user_name = ?{}", idx));
...
pub fn query_sheet1_data(
    ...
    let rows = stmt.query_map(
        ...
        |row| {
            ...
            Ok((lab, project_code, instrument, method, multiplier, quantity, is_gc, coefficient, high_item))
        }
    )?;
...
pub fn query_sheet5_data(
    ...
    let rows = stmt.query_map([start, &end_closed], |row| {
        Ok(PersonRecordRow {
            ...
            quantity: row.get(6)?,
            user_name: row.get(7)?,
            high_item: row.get(8).unwrap_or(None),
        })
    })?;
