...
pub fn batch_import_flat(conn: &Connection, items: &[MethodImportItem]) -> Result<ImportSummary> {
    ...
    for item in items {
        ...
        // 方法名 = 研发项目名/方法名, full_name 存研发项目
        let full_name = format!("{}/{}", item.group_name, item.project_name);
        let existing: Option<i64> = conn.query_row(
            "SELECT id FROM projects WHERE name=?1 AND group_id=?2",
            rusqlite::params![item.method_name, gid], |r| r.get(0)).ok();
        if let Some(pid) = existing {
            conn.execute("UPDATE projects SET full_name=?1, method_type=?2, coefficient=?3 WHERE id=?4",
                rusqlite::params![full_name, item.method_type, item.coefficient, pid])?;
        } else {
    ...
    Ok(ImportSummary {
        total_methods: method_count, total_projects: method_count, total_groups: group_count,
