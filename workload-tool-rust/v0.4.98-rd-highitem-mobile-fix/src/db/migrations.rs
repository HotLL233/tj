...
pub fn run(conn: &rusqlite::Connection) -> Result<()> {
    ...
    conn.execute("ALTER TABLE projects ADD COLUMN project_status TEXT NOT NULL DEFAULT 'ongoing'", []).ok();
    conn.execute("ALTER TABLE projects ADD COLUMN archived_at TEXT DEFAULT NULL", []).ok();
    conn.execute("ALTER TABLE projects ADD COLUMN archived_by TEXT DEFAULT NULL", []).ok();
    ...
    conn.execute("UPDATE rd_record_columns SET sort_order=8 WHERE name='sampling_person' AND sort_order<8", []).ok();
    conn.execute("UPDATE rd_record_columns SET sort_order=9 WHERE name='sampling_time' AND sort_order<9", []).ok();
    conn.execute("UPDATE rd_record_columns SET sort_order=10 WHERE name='status' AND sort_order<10", []).ok();
    conn.execute("UPDATE rd_record_columns SET sort_order=11 WHERE name='notes' AND sort_order<11", []).ok();
    conn.execute(
        "INSERT OR IGNORE INTO rd_record_columns (name, label, data_type, width, sort_order, show_in_list, show_in_form)
...
fn seed_roles(conn: &rusqlite::Connection) -> Result<()> {
    ...
    let seeds: &[(&str, &str, i32, &[&str])] = &[
        ...
        (
            ...
            1,
            &[
                "entry:workload", "entry:sample", "entry:sample-info", "sample:collect",
                "sample-info:collect", "sample-info:complete",
                "manage:projects", "manage:groups", "manage:methods", "manage:trash", "manage:audit", "manage:help",
                "stats:workload:access", "stats:workload:week", "stats:workload:month", "stats:workload:user-log",
                "stats:workload:division", "stats:workload:sheet1", "stats:workload:sheet2", "stats:workload:sheet3",
                "stats:workload:sheet4", "stats:workload:sheet5", "stats:workload:sheet6", "stats:workload:sheet7",
                "stats:workload:sheet8", "stats:workload:sheet9", "stats:workload:sheet10"
            ],
        ),
        (
            "研发送样员",
...
fn seed_default_settings(conn: &rusqlite::Connection) -> Result<()> {
    ...
    // v0.4.63: 高项表 + 项目关联
    conn.execute("CREATE TABLE IF NOT EXISTS high_items (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        name TEXT NOT NULL UNIQUE,
        sort_order INTEGER DEFAULT 0,
        is_active INTEGER NOT NULL DEFAULT 1,
    ...
    conn.execute("INSERT OR IGNORE INTO high_items (name, sort_order) VALUES ('高项', 1)", []).ok();
    ...
    // 新加 high_item TEXT
    conn.execute("ALTER TABLE projects ADD COLUMN high_item TEXT DEFAULT NULL", []).ok();
    conn.execute("ALTER TABLE work_records ADD COLUMN high_item TEXT DEFAULT NULL", []).ok();
    conn.execute("ALTER TABLE rd_work_records ADD COLUMN high_item TEXT DEFAULT NULL", []).ok();
    ...
    let doc_role: std::result::Result<i64, _> = conn.query_row(
        "SELECT id FROM roles WHERE name='文档编辑员'", [], |r| r.get(0)
    );
    if doc_role.is_err() {
        conn.execute(
    ...
    conn.execute("ALTER TABLE sample_info_records ADD COLUMN sampled_by TEXT NOT NULL DEFAULT ''", []).ok();
    conn.execute("ALTER TABLE sample_info_records ADD COLUMN sampled_at TEXT DEFAULT NULL", []).ok();
    conn.execute("ALTER TABLE sample_info_records ADD COLUMN detected_by TEXT NOT NULL DEFAULT ''", []).ok();
    conn.execute_batch(
    ...
    conn.execute("ALTER TABLE work_records ADD COLUMN business_no TEXT NOT NULL DEFAULT ''", []).ok();
    conn.execute("ALTER TABLE work_records ADD COLUMN project_name_snapshot TEXT NOT NULL DEFAULT ''", []).ok();
    conn.execute("ALTER TABLE work_records ADD COLUMN lab_name_snapshot TEXT NOT NULL DEFAULT ''", []).ok();
    conn.execute("ALTER TABLE work_records ADD COLUMN method_name_snapshot TEXT NOT NULL DEFAULT ''", []).ok();
    conn.execute("ALTER TABLE work_records ADD COLUMN high_item_snapshot TEXT NOT NULL DEFAULT ''", []).ok();
    conn.execute("ALTER TABLE work_records ADD COLUMN coefficient_snapshot REAL NOT NULL DEFAULT 1.0", []).ok();
    conn.execute("ALTER TABLE work_records ADD COLUMN updated_at TEXT DEFAULT NULL", []).ok();
    conn.execute("ALTER TABLE rd_work_records ADD COLUMN business_no TEXT NOT NULL DEFAULT ''", []).ok();
    conn.execute("ALTER TABLE rd_work_records ADD COLUMN project_name_snapshot TEXT NOT NULL DEFAULT ''", []).ok();
    conn.execute("ALTER TABLE rd_work_records ADD COLUMN lab_name_snapshot TEXT NOT NULL DEFAULT ''", []).ok();
    conn.execute("ALTER TABLE rd_work_records ADD COLUMN method_name_snapshot TEXT NOT NULL DEFAULT ''", []).ok();
    conn.execute("ALTER TABLE rd_work_records ADD COLUMN high_item_snapshot TEXT NOT NULL DEFAULT ''", []).ok();
    conn.execute("ALTER TABLE rd_work_records ADD COLUMN coefficient_snapshot REAL NOT NULL DEFAULT 1.0", []).ok();
    conn.execute("ALTER TABLE rd_work_records ADD COLUMN updated_at TEXT DEFAULT NULL", []).ok();
    conn.execute("ALTER TABLE sample_info_records ADD COLUMN business_no TEXT NOT NULL DEFAULT ''", []).ok();
    conn.execute_batch(
    ...
    conn.execute("ALTER TABLE audit_log ADD COLUMN business_no TEXT NOT NULL DEFAULT ''", []).ok();
    conn.execute("ALTER TABLE audit_log ADD COLUMN before_json TEXT DEFAULT NULL", []).ok();
    conn.execute("ALTER TABLE audit_log ADD COLUMN after_json TEXT DEFAULT NULL", []).ok();
    conn.execute("ALTER TABLE audit_log ADD COLUMN source TEXT NOT NULL DEFAULT 'application'", []).ok();
    conn.execute("CREATE INDEX IF NOT EXISTS idx_audit_module_time ON audit_log(module, created_at)", []).ok();
    conn.execute("CREATE INDEX IF NOT EXISTS idx_audit_business_no ON audit_log(business_no)", []).ok();
