...
async fn download_template(State(pool): State<DbPool>) -> Result<impl IntoResponse> {
    let _conn = pool.get()?;
    ...
    let response = Response::builder()
        ...
        .header(
            ...
            "attachment; filename=master_data_import_v0.4.94-trace-preview.xlsx; filename*=UTF-8''%E4%B8%BB%E6%95%B0%E6%8D%AE%E4%B8%80%E9%94%AE%E5%AF%BC%E5%85%A5%E6%A8%A1%E6%9D%BF_v0.4.94-trace-preview.xlsx",
        )
...
fn build_simplified_template(conn: &Connection) -> Result<Vec<u8>> {
    ...
    let title = Format::new().set_bold().set_font_size(18).set_font_color(Color::RGB(0x0B6E69));
        ...
        .set_align(FormatAlign::Center);
    {
        ...
        ws.merge_range(0, 0, 0, 1, "主数据一键导入模板 v0.4.94-trace-preview", &title)?;
        let rows = [
    ...
    add_data_sheet(&mut workbook, "主数据", &["对象类型*", "名称*", "所属部门", "全称", "检测类型", "高项", "项目状态", "系数", "倍率", "金额", "排序", "启用", "工作量显示", "研发送样显示", "颜色", "备注"], &[14.0, 24.0, 20.0, 34.0, 26.0, 18.0, 14.0, 10.0, 10.0, 12.0, 10.0, 10.0, 14.0, 14.0, 12.0, 36.0], &[11, 12, 13])?;
    add_data_sheet(&mut workbook, "项目关联", &["项目简称*", "关联实验室*", "关联方法*", "主实验室"], &[26.0, 42.0, 56.0, 24.0], &[])?;

    {
        ...
        let rules = [
            ["对象类型*", "全部", "是", "仅填写：部门、实验室、检测类型、检测方法、研发项目", "无", "检测方法"],
            ["名称*", "全部", "是", "对象的唯一简称；同名数据按导入策略处理", "无", "液相含量测定"],
            ["所属部门", "实验室", "是", "填写程序现有部门或本模板中新建部门的名称", "无", "研究院"],
            ["全称", "检测方法、研发项目", "否", "对象完整名称", "空", "高效液相色谱法含量测定"],
            ["检测类型", "检测方法", "是", "填写一个或多个检测类型；多个值用分号、顿号、逗号或换行分隔", "无", "液相；含量"],
            ["高项", "研发项目", "否", "项目关联的高项文本", "空", "重点项目"],
            ["项目状态", "研发项目", "否", "仅允许：进行中、已归档", "进行中", "进行中"],
            ["系数", "检测方法", "否", "大于等于0的数字", "1", "1.5"],
            ["倍率", "检测方法", "否", "大于等于0的数字", "1", "2"],
            ["金额", "检测方法", "否", "大于等于0的数字", "0", "50"],
            ["排序", "全部", "否", "整数，数值越小显示越靠前", "0", "10"],
            ["启用", "部门、检测方法、研发项目", "否", "仅允许：是、否", "是", "是"],
            ["工作量显示", "实验室", "否", "是否显示在分析检测入口，仅允许：是、否", "是", "是"],
            ["研发送样显示", "实验室", "否", "是否显示在研发送样入口，仅允许：是、否", "是", "是"],
            ["颜色", "部门", "否", "十六进制颜色，格式为 #RRGGBB", "#1976d2", "#2E7D32"],
            ["备注", "检测方法、研发项目", "否", "补充说明，可留空", "空", "常规检测方法"],
            ["项目简称*", "项目关联", "是", "必须与主数据或程序现有项目名称完全一致", "无", "项目A"],
            ["关联实验室*", "项目关联", "是", "一个或多个实验室名称，多值使用分号等支持的分隔符", "无", "实验室401；实验室402"],
            ["关联方法*", "项目关联", "是", "一个或多个方法名称，多值使用分号等支持的分隔符", "无", "液相含量测定；水分测定"],
            ["主实验室", "项目关联", "否", "必须包含在关联实验室中；留空时取第一个关联实验室", "第一个关联实验室", "实验室401"],
        ];
...
fn write_example_sheets(workbook: &mut Workbook) -> std::result::Result<(), XlsxError> {
    ...
    let ws = workbook.add_worksheet();
    ws.set_name("示例数据")?;
    let headers = ["对象类型*", "名称*", "所属部门", "全称", "检测类型", "高项", "项目状态", "系数", "倍率", "金额", "排序", "启用", "工作量显示", "研发送样显示", "颜色", "备注"];
    write_headers(ws, &headers)?;
    let rows = [
        ["部门", "示例研究院", "", "", "", "", "", "", "", "", "10", "是", "", "", "#1976D2", "部门示例"],
        ["实验室", "示例实验室401", "示例研究院", "", "", "", "", "", "", "", "10", "", "是", "是", "", "实验室示例"],
        ["检测类型", "示例液相", "", "", "", "", "", "", "", "", "10", "", "", "", "", "类型示例"],
        ["检测方法", "示例液相含量", "", "示例高效液相色谱含量测定", "示例液相", "", "", "1.5", "1", "50", "10", "是", "", "", "", "方法示例"],
        ["研发项目", "示例项目A", "", "示例研发项目A全称", "", "重点项目", "进行中", "", "", "", "10", "是", "", "", "", "项目示例"],
        ["研发项目", "示例项目B", "", "示例研发项目B全称", "", "一般项目", "已归档", "", "", "", "20", "是", "", "", "", "归档项目示例"],
    ];
    ...
    for (col, width) in [14.0, 24.0, 20.0, 34.0, 26.0, 18.0, 14.0, 10.0, 10.0, 12.0, 10.0, 10.0, 14.0, 14.0, 12.0, 30.0].iter().enumerate() {
        ws.set_column_width(col as u16, *width)?;
    }

    let ws = workbook.add_worksheet();
    ws.set_name("关联示例")?;
    write_headers(ws, &["项目简称*", "关联实验室*", "关联方法*", "主实验室"])?;
    let rows = [
        ["示例项目A", "示例实验室401", "示例液相含量", "示例实验室401"],
        ["示例项目B", "示例实验室401；示例实验室402", "示例液相含量；示例水分测定", "示例实验室401"],
    ];
    for (r, row) in rows.iter().enumerate() {
        for (c, value) in row.iter().enumerate() {
            ws.write_with_format((r + 1) as u32, c as u16, *value, &wrap)?;
...
fn write_data_dictionary(workbook: &mut Workbook, conn: &Connection) -> Result<()> {
    ...
    let specs = [
        ("部门", "SELECT name FROM divisions WHERE is_active=1 ORDER BY sort_order,id", "实验室的所属部门可引用"),
        ("实验室", "SELECT name FROM project_groups WHERE name!='研发项目' ORDER BY sort_order,id", "项目关联和主实验室可引用"),
        ("检测类型", "SELECT name FROM method_types ORDER BY sort_order,id", "检测方法的检测类型可引用"),
        ("检测方法", "SELECT name FROM methods WHERE is_active=1 ORDER BY id", "项目关联方法可引用"),
        ("研发项目", "SELECT name FROM projects WHERE is_active=1 ORDER BY sort_order,id", "项目关联的项目简称可引用"),
    ];
    let wrap = Format::new().set_text_wrap().set_align(FormatAlign::Top);
    let mut row = 1u32;
    for (category, sql, usage) in specs {
        let mut stmt = conn.prepare(sql)?;
        let values = stmt.query_map([], |r| r.get::<_, String>(0))?.collect::<std::result::Result<Vec<_>, _>>()?;
        if values.is_empty() {
...
fn build_template() -> std::result::Result<Vec<u8>, XlsxError> {
...

    {
        ...
        let rows = [
            ...
            ("填写要求", "只填写部门、实验室、检测类型、检测方法、研发项目、项目关联六个工作表。带 * 的字段必填；从第 2 行开始填写，不要修改工作表名称和表头。"),
            ("直接引用", "带下拉箭头的单元格直接引用前面的元数据表。请先完成元数据表，再填写项目关联。"),
            ("多项关联", "每个项目只填写一行。实验室分别选择在关联实验室1～5，方法分别选择在关联方法1～20；不要在一个单元格中拼接多个值。"),
            ...
            ("示例说明", "填写示例工作表仅用于参考，不参与导入。数据表中不要保留不需要导入的示例行。"),
    ...
    add_data_sheet(&mut workbook, "检测方法", &["方法简称*", "方法全称", "检测类型1*", "检测类型2", "检测类型3", "系数", "倍率", "金额", "启用", "备注"], &[26.0, 36.0, 22.0, 22.0, 22.0, 10.0, 10.0, 12.0, 10.0, 36.0], &[8])?;
    add_data_sheet(&mut workbook, "研发项目", &["项目简称*", "项目全称", "高项", "项目状态", "排序", "启用", "备注"], &[26.0, 38.0, 20.0, 14.0, 10.0, 10.0, 36.0], &[5])?;
    let mut relation_headers = vec!["项目简称*", "主实验室"];
    relation_headers.extend(["关联实验室1*", "关联实验室2", "关联实验室3", "关联实验室4", "关联实验室5"]);
    relation_headers.extend(["关联方法1*", "关联方法2", "关联方法3", "关联方法4", "关联方法5", "关联方法6", "关联方法7", "关联方法8", "关联方法9", "关联方法10", "关联方法11", "关联方法12", "关联方法13", "关联方法14", "关联方法15", "关联方法16", "关联方法17", "关联方法18", "关联方法19", "关联方法20"]);
    let relation_widths = vec![26.0, 22.0, 22.0, 22.0, 22.0, 22.0, 22.0, 28.0, 28.0, 28.0, 28.0, 28.0, 28.0, 28.0, 28.0, 28.0, 28.0, 28.0, 28.0, 28.0, 28.0, 28.0, 28.0, 28.0, 28.0, 28.0, 28.0];
    add_data_sheet(&mut workbook, "项目关联", &relation_headers, &relation_widths, &[])?;
    add_data_sheet(&mut workbook, "预检结果", &["工作表", "行号", "对象类型", "对象名称", "处理方式", "状态", "提示"], &[18.0, 10.0, 18.0, 28.0, 16.0, 12.0, 56.0], &[])?;

    workbook.define_name("DepartmentOptions", "=OFFSET('部门'!$A$2,0,0,MAX(1,COUNTA('部门'!$A:$A)-1),1)")?;
    workbook.define_name("LabOptions", "=OFFSET('实验室'!$A$2,0,0,MAX(1,COUNTA('实验室'!$A:$A)-1),1)")?;
    workbook.define_name("MethodTypeOptions", "=OFFSET('检测类型'!$A$2,0,0,MAX(1,COUNTA('检测类型'!$A:$A)-1),1)")?;
    workbook.define_name("MethodOptions", "=OFFSET('检测方法'!$A$2,0,0,MAX(1,COUNTA('检测方法'!$A:$A)-1),1)")?;
    workbook.define_name("ProjectOptions", "=OFFSET('研发项目'!$A$2,0,0,MAX(1,COUNTA('研发项目'!$A:$A)-1),1)")?;

    add_named_list_validation(&mut workbook, "实验室", &[1], "DepartmentOptions")?;
    add_named_list_validation(&mut workbook, "检测方法", &[2, 3, 4], "MethodTypeOptions")?;
    add_named_list_validation(&mut workbook, "项目关联", &[0], "ProjectOptions")?;
    add_named_list_validation(&mut workbook, "项目关联", &[1, 2, 3, 4, 5, 6], "LabOptions")?;
    add_named_list_validation(&mut workbook, "项目关联", &(7u16..=26).collect::<Vec<_>>(), "MethodOptions")?;
    let project_status = DataValidation::new().allow_list_strings(&["进行中", "已归档"])?;
    workbook.worksheet_from_name("研发项目")?.add_data_validation(1, 3, 1000, 3, &project_status)?;

    {
        let ws = workbook.add_worksheet();
        ws.set_name("填写示例")?;
        write_headers(ws, &["步骤", "工作表", "示例内容", "说明"])?;
        let examples = [
            ["1", "部门", "研究院｜10｜#1976D2｜是", "先建立部门"],
            ["2", "实验室", "液相实验室｜研究院｜10｜是｜是", "所属部门从部门表下拉选择"],
            ["3", "检测类型", "液相｜10", "检测方法引用此类型"],
            ["4", "检测方法", "方法001｜高效液相色谱含量测定｜液相｜1｜1｜0｜是", "检测类型从检测类型表下拉选择"],
            ["5", "研发项目", "项目001｜项目001全称｜高项001｜进行中｜10｜是", "项目状态默认为进行中"],
            ["6", "项目关联", "项目001｜液相实验室｜液相实验室｜方法001", "一行一个项目，每个关联项单独选择"],
            ["7", "项目关联", "项目002｜液相实验室｜液相实验室/质谱实验室｜方法001/方法002/方法003", "多实验室、多方法分别填写在后续选择列"],
        ];
        let wrap = Format::new().set_text_wrap().set_align(FormatAlign::Top);
...
fn add_data_sheet(
    ...
    if name == "主数据" {
        let entity_validation = DataValidation::new().allow_list_strings(&["部门", "实验室", "检测类型", "检测方法", "研发项目"])?;
...
fn parse_workbook_path(path: &std::path::Path) -> Result<(ParsedData, Vec<ImportIssue>)> {
    ...
    if sheet_names.contains("主数据") {
        return parse_compact_workbook(&mut workbook);
    }
    ...
    for row in methods {
        ...
        if method_types.is_empty() {
            push_error(&mut issues, "检测方法", row.row, "检测方法", &name, "至少选择一个检测类型");
        }
    ...
    for row in relations {
        ...
        let labs = collect_numbered_values(&row, &["关联实验室*", "关联实验室"], "关联实验室", 5);
        let methods = collect_numbered_values(&row, &["关联检测方法*", "关联检测方法", "关联方法*", "关联方法"], "关联方法", 20);
        if labs.is_empty() {
            push_error(&mut issues, "项目关联", row.row, "项目关联", &project, "至少选择一个关联实验室");
        }
        if methods.is_empty() {
            push_error(&mut issues, "项目关联", row.row, "项目关联", &project, "至少选择一个关联方法");
        }
...
fn parse_compact_workbook<R: std::io::Read + std::io::Seek>(workbook: &mut calamine::Sheets<R>) -> Result<(ParsedData, Vec<ImportIssue>)> {
    let mut data = ParsedData::default();
    let mut issues = Vec::new();

    let main_rows = read_sheet_rows(&workbook.worksheet_range("主数据").map_err(workbook_error)?)?;
    for row in main_rows {
        let entity = required_any(&row, &["对象类型*", "对象类型", "类型"], "主数据", "主数据", &mut issues);
        let name = required_any(&row, &["名称*", "名称", "简称*"], "主数据", &entity, &mut issues);
        if entity.is_empty() || name.is_empty() { continue; }
        if entity.contains("部门") && !entity.contains("实验") {
            ...
            data.departments.push(DepartmentRow {
                ...
                color: if color.is_empty() { "#1976d2".into() } else { color },
                is_active: parse_bool(&row, "启用", true, "主数据", &entity, &mut issues),
            });
        } else if entity.contains("实验室") {
            let department = required_any(&row, &["所属部门*", "所属部门"], "主数据", "实验室", &mut issues);
            if department.is_empty() { continue; }
            data.labs.push(LabRow {
                row: row.row,
                name,
                department,
            ...
            let method_types = split_multi(&required_any(&row, &["检测类型*", "检测类型"], "主数据", "检测方法", &mut issues));
            if method_types.is_empty() { continue; }
            data.methods.push(MethodRow {
                row: row.row,
                name,
            ...
            data.projects.push(ProjectRow {
                ...
                project_status: parse_project_status(&row, "项目状态", "主数据", &name, &mut issues),
                notes: value_any(&row, &["备注"]),
            });
        } else {
            push_error(&mut issues, "主数据", row.row, "主数据", &name, "对象类型仅支持部门、实验室、检测类型、检测方法、研发项目");
        }
    }

    let relation_rows = read_sheet_rows(&workbook.worksheet_range("项目关联").map_err(workbook_error)?)?;
    for row in relation_rows {
        let project = required_any(&row, &["项目简称*", "项目简称", "项目名称*", "项目名称"], "项目关联", "项目关联", &mut issues);
        let labs = split_multi(&required_any(&row, &["关联实验室*", "关联实验室"], "项目关联", "项目关联", &mut issues));
        let methods = split_multi(&required_any(&row, &["关联方法*", "关联方法", "关联检测方法*", "关联检测方法"], "项目关联", "项目关联", &mut issues));
        if project.is_empty() || labs.is_empty() || methods.is_empty() { continue; }
        let primary = value_any(&row, &["主实验室"]);
        if !primary.is_empty() && !labs.iter().any(|x| x == &primary) {
            push_error(&mut issues, "项目关联", row.row, "项目关联", &project, "主实验室必须包含在关联实验室中");
        }
        data.relations.push(RelationRow {
    ...
    check_duplicates(&data.departments, |x| (&x.name, x.row), "主数据", "部门", &mut issues);
    check_duplicates(&data.labs, |x| (&x.name, x.row), "主数据", "实验室", &mut issues);
    check_duplicates(&data.method_types, |x| (&x.name, x.row), "主数据", "检测类型", &mut issues);
    check_duplicates(&data.methods, |x| (&x.name, x.row), "主数据", "检测方法", &mut issues);
    check_duplicates(&data.projects, |x| (&x.name, x.row), "主数据", "研发项目", &mut issues);
    check_duplicates(&data.relations, |x| (&x.project, x.row), "项目关联", "项目关联", &mut issues);
...
fn required_any(row: &SheetRow, headers: &[&str], sheet: &str, entity_type: &str, issues: &mut Vec<ImportIssue>) -> String {
    let result = value_any(row, headers);
    if result.is_empty() {
        push_error(issues, sheet, row.row, entity_type, "", &format!("必填字段 {:?} 不能为空", headers));
    }
    result
}
...
fn parse_project_status(row: &SheetRow, header: &str, sheet: &str, name: &str, issues: &mut Vec<ImportIssue>) -> String {
    let raw = value(row, header);
    match raw.trim().to_ascii_lowercase().as_str() {
        "" | "进行中" | "ongoing" => "ongoing".into(),
        "已归档" | "归档" | "archived" => "archived".into(),
        _ => {
            push_error(issues, sheet, row.row, "研发项目", name, "字段「项目状态」只能填写进行中或已归档");
            "ongoing".into()
        }
    }
}
        ...
        match (existing, mode) {
            (Some(id), ImportMode::Upsert) => {
                tx.execute(
                    ...
                    rusqlite::params![row.full_name, row.high_item, row.sort_order, row.is_active, row.notes, primary_group_id, row.project_status, id],
                )?;
                counts.updated += 1;
            }
            (Some(_), ImportMode::Skip) => counts.skipped += 1,
...
mod tests {
    ...
    fn generated_template_has_all_required_sheets() {
        ...
        for sheet in ["使用说明", "字段字典", "部门", "实验室", "检测类型", "检测方法", "研发项目", "项目关联", "预检结果", "填写示例"] {
    ...
    #[test]
    fn separate_sheet_template_parses_numbered_relations() {
        let mut workbook = Workbook::new();
        add_data_sheet(&mut workbook, "部门", &["部门名称*", "排序", "颜色", "启用"], &[20.0; 4], &[3]).expect("departments");
        add_data_sheet(&mut workbook, "实验室", &["实验室名称*", "所属部门*", "排序", "工作量显示", "研发送样显示"], &[20.0; 5], &[3, 4]).expect("labs");
        add_data_sheet(&mut workbook, "检测类型", &["类型名称*", "排序"], &[20.0; 2], &[]).expect("types");
        add_data_sheet(&mut workbook, "检测方法", &["方法简称*", "方法全称", "检测类型1*", "检测类型2", "检测类型3", "系数", "倍率", "金额", "启用", "备注"], &[20.0; 10], &[8]).expect("methods");
        add_data_sheet(&mut workbook, "研发项目", &["项目简称*", "项目全称", "高项", "项目状态", "排序", "启用", "备注"], &[20.0; 7], &[5]).expect("projects");
        add_data_sheet(&mut workbook, "项目关联", &["项目简称*", "主实验室", "关联实验室1*", "关联实验室2", "关联方法1*", "关联方法2"], &[20.0; 6], &[]).expect("relations");
        add_data_sheet(&mut workbook, "预检结果", &["提示"], &[30.0], &[]).expect("preview");

        workbook.worksheet_from_name("部门").unwrap().write(1, 0, "测试部门").unwrap();
        workbook.worksheet_from_name("实验室").unwrap().write(1, 0, "实验室01").unwrap().write(1, 1, "测试部门").unwrap();
        workbook.worksheet_from_name("检测类型").unwrap().write(1, 0, "液相").unwrap();
        workbook.worksheet_from_name("检测方法").unwrap().write(1, 0, "方法01").unwrap().write(1, 2, "液相").unwrap();
        workbook.worksheet_from_name("研发项目").unwrap().write(1, 0, "项目01").unwrap().write(1, 3, "进行中").unwrap();
        workbook.worksheet_from_name("项目关联").unwrap().write(1, 0, "项目01").unwrap().write(1, 1, "实验室01").unwrap().write(1, 2, "实验室01").unwrap().write(1, 4, "方法01").unwrap();

        let path = std::env::temp_dir().join(format!("master_numbered_test_{}.xlsx", uuid::Uuid::new_v4()));
        workbook.save(&path).expect("save workbook");
        let (data, issues) = parse_workbook_path(&path).expect("parse workbook");
        let _ = std::fs::remove_file(path);
        assert!(issues.iter().all(|issue| issue.level != "error"), "issues: {issues:?}");
        assert_eq!(data.departments.len(), 1);
        assert_eq!(data.methods[0].method_types, vec!["液相"]);
        assert_eq!(data.relations[0].labs, vec!["实验室01"]);
        assert_eq!(data.relations[0].methods, vec!["方法01"]);
    ...
    fn transaction_import_writes_master_data_and_links() {
        ...
        let data = ParsedData {
            ...
            projects: vec![ProjectRow { row: 2, name: "测试项目".into(), full_name: "测试项目全称".into(), high_item: Some("高项A".into()), sort_order: 20, is_active: true, project_status: "archived".into(), notes: "".into() }],
        ...
        let project_status: String = conn.query_row("SELECT project_status FROM projects WHERE name='测试项目'", [], |row| row.get(0)).expect("project status");
