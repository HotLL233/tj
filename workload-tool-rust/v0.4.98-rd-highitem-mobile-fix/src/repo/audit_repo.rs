...
pub fn log_structured_on_conn(
    ...
    conn.execute(
        "INSERT INTO audit_log
         (action,table_name,record_id,user_name,detail,module,business_no,before_json,after_json,source,created_at)
...
pub fn list(
    ...
    let base = "SELECT id,action,table_name,record_id,user_name,detail,module,created_at,
                       business_no,before_json,after_json,source FROM audit_log";
    let sql = format!("{}{} ORDER BY created_at DESC,id DESC LIMIT {} OFFSET {}", base, where_clause, page_size, offset);
    ...
    let rows = stmt.query_map(rusqlite::params_from_iter(params.iter()), |row| {
        ...
        Ok(AuditLogResponse {
            ...
            created_at: row.get(7)?, business_no: row.get::<_, String>(8).unwrap_or_default(),
            ...
            source: row.get::<_, String>(11).unwrap_or_else(|_| "application".into()),
        })
    })?;
