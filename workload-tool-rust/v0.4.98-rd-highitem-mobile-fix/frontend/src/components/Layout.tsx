    ...
    <Dialog open={aboutOpen} onClose={() => setAboutOpen(false)} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: '2px' } }}>
      ...
      <DialogContent>
        ...
        <Alert severity="info" sx={{ mb: 2, borderRadius: '2px' }}>
          <Typography variant="subtitle2" sx={{ fontWeight: 600, mb: 0.5 }}>v0.4.98 研发送样高项与手机布局修复版</Typography>
          <Typography variant="body2">研发送样记录显示创建时保存的关联高项快照。</Typography>
          <Typography variant="body2">今日记录和研发送样记录页面同步补全高项。</Typography>
          <Typography variant="body2">手机端实验室、项目和检测类型改为整行显示。</Typography>
          <Typography variant="body2">手机端操作按钮保持单行等宽，减少无效占位。</Typography>
        </Alert>
