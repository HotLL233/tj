...
fn row_to_method(row: &rusqlite::Row) -> rusqlite::Result<MethodResponse> {
    let type_ids_str: String = row.get::<_, String>(6).unwrap_or_default();
    let type_names_str: String = row.get::<_, String>(7).unwrap_or_default();
    Ok(MethodResponse {
        id: row.get(0)?,
        name: row.get(1)?,
        full_name: row.get(2)?,
...
pub fn create(pool: &DbPool, body: &MethodCreate) -> Result<MethodResponse> {
    ...
    conn.execute(
        "INSERT INTO methods (name, full_name, coefficient, notes) VALUES (?1,?2,?3,?4)",
        rusqlite::params![body.name, fn_, cf, nt],
    )?;
