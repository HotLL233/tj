...
export const downloadMasterImportTemplate = async (): Promise<void> => {
  ...
  anchor.href = url;
  anchor.download = '涓绘暟鎹竴閿鍏ユā鏉縚v0.4.83.xlsx';
  anchor.click();
...
async function downloadFile(url: string, params: Record<string, any>, filename: string): Promise<void> {
  ...
  if (!res.ok) {
    ...
    let msg = `瀵煎嚭澶辫触 (HTTP ${res.status})`;
    try { const j = JSON.parse(txt); if (j.message) msg = j.message; } catch {}
    throw new Error(msg);
  }
  const blob = await res.blob();
  if (blob.size === 0) throw new Error('瀵煎嚭鏂囦欢涓虹┖');
...
export const exportExcel = (params: { start?: string; end?: string; group_id?: number }): Promise<void> =>
  downloadFile('/api/export/excel', params, `鏍峰搧绠＄悊_${params.start?.substring(0, 10) ?? ''}_${params.end?.substring(0, 10) ?? ''}.xlsx`);
...
export const updateRdRecord = (id: number, data: { user_name?: string; quantity?: number; recorded_at?: string; multiplier?: number; project_id?: number; method_id?: number | null; batch_no?: string; notes?: string; high_item?: string | null }): Promise<ApiResponse<WorkRecord>> =>
...
export const exportRdExcel = (params: { start?: string; end?: string; group_id?: number }): Promise<void> =>
  downloadFile('/api/rd-export/excel', params, `鐮斿彂閫佹牱_${params.start?.substring(0, 10) ?? ''}_${params.end?.substring(0, 10) ?? ''}.xlsx`);
...
export const exportSampleInfo = (params: { start?: string; end?: string }): Promise<void> =>
  downloadFile('/api/sample-info/export', params, `鏍峰搧淇℃伅鐧昏_${params.start?.substring(0, 10) ?? ''}_${params.end?.substring(0, 10) ?? ''}.xlsx`);
