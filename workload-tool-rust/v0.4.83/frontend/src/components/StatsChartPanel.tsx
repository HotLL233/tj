...
const COLORS = ['#1976d2', '#43a047', '#f57c00', '#8e24aa', '#00897b', '#e91e63', '#546e7a', '#6d4c41'];
...
const StatsChartPanel: React.FC<Props> = props => {
  const { active, title, loading, filters } = props;
  ...
  return (
    <Paper data-testid="stats-detail-chart" elevation={0} sx={{ mb: 2.5, p: { xs: 1.5, md: 2 }, borderRadius: '2px', border: '1px solid rgba(0,0,0,0.08)', position: 'relative', overflow: 'hidden' }}>
            ...
            <ResponsiveContainer width="100%" height="100%">
              {chartType === 'line' ? (
                <LineChart data={data} margin={{ top: 12, right: 24, bottom: 16, left: 4 }}>
                  ...
                  <Tooltip /><Line type="monotone" dataKey="value" name={metricLabels[metric]} stroke={color} strokeWidth={2.5} dot={{ r: 3 }} activeDot={{ r: 5 }} isAnimationActive={false} />
                </LineChart>
              ) : chartType === 'bar' ? (
