...
const getRecordColumnPercent = (name: string) => {
...
const RdRecordsPage: React.FC = () => {
  ...
  return (
    ...
    {loading && records.length === 0 ? (
      ...
      <TableContainer component={Paper} variant="outlined" sx={{ borderRadius: R, boxShadow: 'none', '& .MuiPaper-root': { borderRadius: R }, overflowX: 'hidden' }}>
            ...
            <TableRow sx={{ bgcolor: 'rgba(230,81,0,0.06)' }}>
              {columns.map(col => {
                ...
                <TableCell key={col.name} sx={{
                  fontWeight: 700, fontSize: '0.78rem', whiteSpace: 'normal',
                  overflowWrap: 'anywhere', wordBreak: 'break-word',
                  width: getRecordColumnPercent(col.name),
              ...
              <TableCell sx={{ fontWeight: 700, fontSize: '0.78rem', whiteSpace: 'normal', width: '5%', px: 0.75, py: 1 }}>操作</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {records.map((rec, idx) => {
