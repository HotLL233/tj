...
const EntryPage: React.FC = () => {
  ...
  const [samplerTarget, setSamplerTarget] = useState<number>(0);
  ...
  const handleSamplerSubmit = async () => {
    if (!samplerName.trim()) { setSnackMsg('请输入取样人'); setSnackErr(true); return; }
    try {
      const r = await sampleRdRecord(samplerTarget, samplerName.trim());
      if (r.code === 0) {
        setSnackMsg('取样成功'); setSnackErr(false);
        setSamplerOpen(false);
        refreshRecords();
      } else {
        setSnackMsg(r.message); setSnackErr(true);
      }
  ...
  return (<Box>
                ...
                <TableRow key={rec.id} hover sx={{ '&:last-child td': { borderBottom: 0 } }}>
                  ...
                  <TableCell sx={{ fontSize: '0.8rem', whiteSpace: 'nowrap' }}>
                    {extractInstrumentFromMethodName(rec.method_name || '') ? (
                      <Chip label={extractInstrumentFromMethodName(rec.method_name || '')} size="small" sx={{ bgcolor: '#9c27b0', color: '#fff', borderRadius: R, height: 20, fontSize: '0.7rem' }} />
                    ) : '-'}
                  </TableCell>
                  ...
                  <TableCell sx={{ fontSize: '0.8rem', whiteSpace: 'nowrap' }}>
                    {isSampled ? (
                      ...
                      <Button size="small" variant="outlined" sx={{ borderRadius: R, fontSize: '0.7rem', py: 0, minWidth: 56 }}
                        onClick={() => { setSamplerTarget(rec.id); setSamplerName(''); setSamplerOpen(true); }}>
                        取样
                      </Button>
                    )}
                  </TableCell>
    ...
    <Dialog open={samplerOpen} onClose={() => setSamplerOpen(false)} maxWidth="xs" fullWidth>
      <DialogTitle sx={{ fontWeight: 700, fontSize: '1rem' }}>填写取样人</DialogTitle>
      <DialogContent>
        <TextField
          autoFocus
          fullWidth
          ...
          onKeyDown={e => { if (e.key === 'Enter') handleSamplerSubmit(); }}
          sx={{ mt: 1, '& .MuiOutlinedInput-root': { borderRadius: R } }}
        />
      </DialogContent>
      <DialogActions>
        <Button onClick={() => setSamplerOpen(false)} sx={{ borderRadius: R, color: 'text.secondary' }}>取消</Button>
        <Button onClick={handleSamplerSubmit} variant="contained" sx={{ borderRadius: R, bgcolor: '#ef6c00', '&:hover': { bgcolor: '#e65100' } }}>确定取样</Button>
      </DialogActions>
    </Dialog>
  </Box>);
};
    ...
    <Box sx={{
      ...
      <Box sx={{ flex: 1, width: isMobile ? '100%' : undefined }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap' }}>
          ...
          <Chip label={`×${(method.coefficient ?? 1).toFixed(1)}`} size="small" variant="outlined" sx={{ borderRadius: '2px', height: 22, fontSize: '0.7rem', borderColor: '#ef6c00', color: '#ef6c00', fontWeight: 600 }} />
