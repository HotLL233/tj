...
fn user_role_ids_on_conn(conn: &rusqlite::Connection, user_id: i64) -> Result<Vec<i64>> {
    let mut stmt = conn.prepare("SELECT role_id FROM user_roles WHERE user_id = ?1 ORDER BY role_id")?;
...
pub fn create(pool: &DbPool, data: &UserCreate, password_hash: &str) -> Result<User> {
    ...
    for rid in &data.role_ids {
        tx.execute("INSERT OR IGNORE INTO user_roles (user_id, role_id) VALUES (?1, ?2)", rusqlite::params![id, rid]).ok();
    }
...
pub fn update(pool: &DbPool, id: i64, data: &UserUpdate, user_name: &str) -> Result<User> {
    ...
    if let Some(ref role_ids) = data.role_ids {
        tx.execute("DELETE FROM user_roles WHERE user_id = ?1", [id]).ok();
        for rid in role_ids {
            tx.execute("INSERT OR IGNORE INTO user_roles (user_id, role_id) VALUES (?1, ?2)", rusqlite::params![id, rid]).ok();
        }
        changes.push("角色已更新".into());
