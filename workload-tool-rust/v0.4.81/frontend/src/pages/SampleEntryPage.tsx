...
const getRdRecordColumnWidth = (key: string, configured?: number) => {
  ...
  return fixed[key] || Math.min(Math.max(configured || 110, 80), 220);
};
...
const SampleEntryPage: React.FC = () => {
  ...
  const todayRecordsTableMinWidth = visibleLayoutFields.reduce(
    (sum, field) => sum + getRdRecordColumnWidth(field.key, field.width),
    tableConfig.seq_column_width + 76,
  );
      ...
      <Box sx={{ mt: 3 }}>
        ...
        {recordsLoading && todayRecords.length === 0 ? (
          ...
          <TableContainer component={Paper} variant="outlined" sx={{ borderRadius: R, boxShadow: 'none', overflowX: 'auto' }}>
            <Table size="small" sx={{ width: 'max-content', minWidth: todayRecordsTableMinWidth, tableLayout: 'fixed' }}>
              <TableHead>
                <TableRow sx={{ bgcolor: 'rgba(230,81,0,0.06)' }}>
                  <TableCell sx={{ fontWeight: 700, fontSize: '0.8rem', whiteSpace: 'nowrap', width: tableConfig.seq_column_width, textAlign: 'center' }}>序号</TableCell>
                  {visibleLayoutFields.map(field => (
                    <TableCell
                      ...
                      sx={{
                        ...
                        fontSize: '0.8rem',
                        whiteSpace: 'nowrap',
                        width: getRdRecordColumnWidth(field.key, field.width),
                      }}
                    >
                      {field.label}
                    </TableCell>
                  ))}
                  <TableCell sx={{ fontWeight: 700, fontSize: '0.8rem', whiteSpace: 'nowrap', width: 76 }}>高项</TableCell>
                </TableRow>
              </TableHead>
              <TableBody>
