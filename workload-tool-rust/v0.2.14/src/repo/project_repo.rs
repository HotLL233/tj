...
/// v0.2.14: 三层级导入,按子类型分组
/// grouped: { (实验室, 研发项目): [(子类型, 方法名), ...] }
/// 子类型: "检测方法"(液相/气相) / "理化方法" / "ICP" / "热分析" / "其他"
pub fn batch_import_3level(
    conn: &Connection,
    ...
    for ((lab, proj), methods) in grouped {
        ...
        // 第二/第三层级: 按子类型(检测方法/理化方法/ICP/热分析/其他)存
        for (subtype, mname) in methods {
            let existing: Option<i64> = conn.query_row(
                "SELECT id FROM projects WHERE name=?1 AND group_id=?2 AND method_type=?3",
                rusqlite::params![mname, gid, subtype], |r| r.get(0)).ok();
            if existing.is_none() {
                conn.execute("INSERT INTO projects (group_id, name, method_type, full_name) VALUES (?1,?2,?3,?4)",
                    rusqlite::params![gid, mname, subtype, format!("{}/{}", lab, proj)])?;
                method_count += 1;
            }
            *type_counter.entry(subtype.clone()).or_insert(0) += 1;
        }
    }
