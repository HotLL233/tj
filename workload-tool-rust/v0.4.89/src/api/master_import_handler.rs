...
async fn download_template(State(pool): State<DbPool>) -> Result<impl IntoResponse> {
    let conn = pool.get()?;
    let bytes = build_simplified_template(&conn)?;
    let response = Response::builder()
        ...
        .header(
            ...
            "attachment; filename=master_data_import_v0.4.89.xlsx; filename*=UTF-8''%E4%B8%BB%E6%95%B0%E6%8D%AE%E5%AF%BC%E5%85%A5%E6%A8%A1%E6%9D%BF_v0.4.89.xlsx",
        )
...
fn build_simplified_template(conn: &Connection) -> Result<Vec<u8>> {
        ...
        .set_align(FormatAlign::Center);
    {
        ...
        ws.merge_range(0, 0, 0, 1, "主数据一键导入模板 v0.4.89", &title)?;
        let rows = [
...
fn build_template() -> std::result::Result<Vec<u8>, XlsxError> {
...

    {
        ...
        ws.set_column_width(1, 90)?;
        ws.merge_range(0, 0, 0, 1, "主数据一键导入模板 v0.4.77", &title)?;
        let rows = [
...
mod tests {
    ...
    fn generated_template_has_all_required_sheets() {
        ...
        let bytes = build_simplified_template(&conn).expect("template");
        ...
        for sheet in ["使用说明", "主数据", "项目关联", "填写规则", "示例数据", "关联示例", "数据字典"] {
