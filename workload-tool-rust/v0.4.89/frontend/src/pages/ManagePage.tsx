...
type TV = 'projects' | 'groups' | 'methods' | 'divisions' | 'master-import' | 'trash' | 'audit' | 'backup' | 'help' | 'sampleinfo' | 'users' | 'roles' | 'layouts' | 'theme' | 'home' | 'stats' | 'forms' | 'exports';

const ROUTABLE_TABS: TV[] = ['projects', 'groups', 'divisions', 'master-import', 'methods', 'trash', 'audit', 'backup', 'help', 'sampleinfo', 'users', 'layouts', 'forms', 'exports'];
...
const ManagePage: React.FC = () => {
  ...
  const auditLogWidths = useMemo(() => getAdaptiveColumnWidths(al, [
    ...
    { key: 'table_name', header: '操作对象', min: 78, max: 130, getValue: r => TL[r.table_name] || r.table_name },
    { key: 'record_id', header: '记录ID', fixed: 66, getValue: r => r.record_id },
    { key: 'user_name', header: '操作人', min: 62, max: 110, getValue: r => r.user_name },
    { key: 'detail', header: '详情', min: 160, max: 300, getValue: r => r.detail },
  ]), [al]);
          ...
          <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr', md: '1fr 1fr 1fr' }, gap: 1.5, mb: 3 }}>
            {visibleCards.map(c => (
              <Paper
                key={c.key}
                elevation={0}
                onClick={() => navigate(c.key === 'roles' ? '/manage/roles' : `/manage/${c.key}`)}
                sx={{ p: isMobile ? 1.5 : 2.5, borderRadius: R, cursor: 'pointer', border: '2px solid rgba(0,0,0,0.06)', transition: 'all 0.2s', '&:hover': { borderColor: '#f4511e', boxShadow: '0 4px 24px rgba(244,81,30,0.12)' } }}
              >
                <Box sx={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', alignItems: isMobile ? 'center' : 'center', gap: isMobile ? 0.5 : 1.5, mb: 0.5, textAlign: isMobile ? 'center' : 'left' }}>
                  <Box sx={{ color: '#6b7280' }}>{c.icon}</Box>
        ...
        <TableContainer component={Paper} className="table-responsive" sx={tSx}>
          <Table size="small" sx={adaptiveTableSx}>
            <TableHead><TableRow>
              ...
              <TableCell sx={{ ...adaptiveCellSx(auditLogWidths.table_name), fontWeight: 600 }}>操作对象</TableCell>
              <TableCell sx={{ ...adaptiveCellSx(auditLogWidths.record_id), fontWeight: 600 }}>记录ID</TableCell>
            ...
            <TableBody>{al.map(l => (
              <TableRow key={l.id} hover>
                ...
                <TableCell sx={adaptiveCellSx(auditLogWidths.table_name)}>{TL[l.table_name] || l.table_name}</TableCell>
                <TableCell sx={adaptiveCellSx(auditLogWidths.record_id)}>{l.record_id}</TableCell>
                ...
                <TableRow key={r.id} hover>
                  {siEditId === r.id ? (
                    ...
                    <>
                      ...
                      <TableCell sx={adaptiveCellSx(siRecordWidths.detection_type)}>{r.detection_type}</TableCell>
                      <TableCell sx={adaptiveCellSx(siRecordWidths.status)}><Chip label={r.status} size="small" color={r.status === '检测完成' ? 'default' : 'warning'} /></TableCell>
                      <TableCell sx={adaptiveCellSx(siRecordWidths.detection_date)}>{r.detection_date || '-'}</TableCell>
