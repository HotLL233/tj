...
const SampleEntryPage: React.FC = () => {
  ...
  const recordTableMinWidth = useMemo(() => {
    return (tableConfig.seq_column_width || 50)
      + recordLayoutFields.reduce((sum, field) => sum + getRecordFieldWidth(field.key, field.width), 0)
      + 90;
  }, [tableConfig.seq_column_width, recordLayoutFields]);

  const recordAdaptiveWidths = useMemo(() => {
    return getAdaptiveColumnWidths(todayRecords, recordLayoutFields.map(field => ({
      key: field.key,
      header: field.label,
  ...
  }, [todayRecords, recordLayoutFields, labName, divs]);
  ...
  const pageContent = (
    ...
    <Box sx={{ p: 2 }}>
      ...
      <Paper elevation={0} sx={{ p: 1.5, mb: 2, borderRadius: R, border: '2px solid #2e7d32', background: 'linear-gradient(145deg,#ffffff,#f1f8e9)' }}>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 1.5, flexWrap: 'wrap', mb: 1.25 }}>
          ...
          <TextField
            ...
            sx={{ width: isMobile ? '100%' : 220, '& .MuiOutlinedInput-root': { borderRadius: R, minHeight: 38 } }}
          />
          <Button variant="contained" size="small" startIcon={<SendIcon />} onClick={handleSubmit}
            sx={{ borderRadius: R, bgcolor: '#2e7d32', '&:hover': { bgcolor: '#1b5e20' }, minHeight: 38 }}
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1, mb: 1.25 }}>
          <Box sx={{
            display: 'grid',
            gridTemplateColumns: '42px 54px repeat(5, minmax(110px, 1fr))',
            alignItems: 'center',
            gap: 1,
            px: 1,
            py: 0.75,
              ...
              <Paper key={row.id} variant="outlined" sx={{ borderRadius: R, boxShadow: 'none', overflow: 'hidden' }}>
                <Box sx={{
                  ...
                  {topEntryFields.map(field => (
                    <Box key={field.key}>{renderEntryInput(field, row, availableTypes, availableMethods)}</Box>
                  ))}
                </Box>
                <Box sx={{
                  display: 'grid',
                  gridTemplateColumns: 'minmax(320px, 2.2fr) 86px minmax(120px, 0.8fr) minmax(220px, 1.5fr)',
                  gap: 1,
                  px: 1,
                  py: 0.75,
                  bgcolor: '#fff',
            ...
            <Table size="small" sx={adaptiveTableSx}>
              <TableHead>
                <TableRow sx={{ bgcolor: 'rgba(230,81,0,0.06)' }}>
                  ...
                  {recordLayoutFields.map(field => (
                    <TableCell
                      ...
                      sx={{
                        fontWeight: 700,
                        fontSize: '0.8rem',
                        ...adaptiveCellSx(recordAdaptiveWidths[field.key] || getRecordFieldWidth(field.key, field.width)),
                        px: 0.75,
                      }}
                    >
                      {field.label}
                  ...
                  <TableCell sx={{ fontWeight: 700, fontSize: '0.78rem', whiteSpace: 'normal', width: '7%', px: 0.75 }}>高项</TableCell>
                </TableRow>
              </TableHead>
              <TableBody>
                {todayRecords.map((rec, idx) => (
                  <TableRow key={rec.id} hover
                    ...
                    <TableCell sx={{ ...rdRecordCellSx, whiteSpace: 'nowrap' }}>{(rec as any).high_item || '-'}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
