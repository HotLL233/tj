    ...
    <Dialog open={aboutOpen} onClose={() => setAboutOpen(false)} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: '2px' } }}>
      ...
      <DialogContent>
        ...
        <Alert severity="info" sx={{ mb: 2, borderRadius: '2px' }}>
          <Typography variant="subtitle2" sx={{ fontWeight: 600, mb: 0.5 }}>v0.4.89 更新内容</Typography>
          <Typography variant="body2">• 主数据模板增加详细规则、示例和程序数据字典</Typography>
          <Typography variant="body2">• 修复模板下载文件名乱码和项目状态导入</Typography>
          <Typography variant="body2">• 研发项目按进行中与已归档双栏管理</Typography>
          <Typography variant="body2">• 归档项目自动退出前台选项并支持重新启用</Typography>
          <Typography variant="body2">• 数据库备份与全量备份统一支持手动和定时执行</Typography>
          <Typography variant="body2">• 全量备份包含数据库、附件、配置和校验清单</Typography>
          <Typography variant="body2">• 恢复前自动校验并生成安全备份，重启后生效</Typography>
        </Alert>
