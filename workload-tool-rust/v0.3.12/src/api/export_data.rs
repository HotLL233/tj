...
pub fn query_sheet1_data(
    ...
    // 修复 v0.3.9: 改为 FROM work_records（而非 projects），避免无记录项目产生幽灵行
    // 原来的 FROM projects p LEFT JOIN work_records 会导致：
    //   1. 所有项目都列出（包括没有任何记录的）
    //   2. 项目关联多实验室时，标量子查询 LIMIT 1 可能返回不同值，导致同一项目出现多次
    let mut sql = String::from(
        "SELECT
                (SELECT pg2.name FROM project_lab_links pll2
                 JOIN project_groups pg2 ON pll2.group_id = pg2.id AND pg2.name != '研发项目'
                 WHERE pll2.project_id = p.id LIMIT 1) as lab_name,
...
pub fn query_sheet4_data(
    ...
    let sql =
        "SELECT (SELECT pg2.name FROM project_lab_links pll2
                 JOIN project_groups pg2 ON pll2.group_id = pg2.id AND pg2.name != '研发项目'
                 WHERE pll2.project_id = p.id LIMIT 1) AS lab_name,
                p.name AS project_name,
                COALESCE(m.full_name, m.name),
