...
async fn sample(State(pool): State<DbPool>, headers: HeaderMap, Path(id): Path<i64>, Json(body): Json<RdSampleUpdate>) -> Result<Json<ApiResponse<RdRecordResponse>>> {
    let user_name = extract_claims_username(&headers);
    let result = rd_record_service::sample(&pool, id, &body)?;
    let conn = pool.get()?;
    audit_repo::log_on_conn_with_module(
        &conn, "sample", "rd_work_records", Some(id),
        &user_name, &format!("研发送样记录#{} 取样人「{}」", id, body.sampler), "rd",
    )?;
    Ok(Json(ApiResponse::ok(result)))
}
