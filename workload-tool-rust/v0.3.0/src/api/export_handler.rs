...
fn build_daily(ws: &mut rust_xlsxwriter::Worksheet, conn: &rusqlite::Connection, start: Option<&str>, end: Option<&str>, group_id: Option<i64>) -> Result<()> {
    ...
    for wd in &date_order { for gn in date_data[wd].keys() { for (pc, pr) in &date_data[wd][gn] {
        let coeff = pr.coeff;
        for (ic, ml, q) in &pr.lc { rows.push((wd.clone(), format!("{}/{}", gn, pc), ic.clone(), ml.clone(), *q, false, coeff)); }
        for (ic, ml, q) in &pr.gc { rows.push((wd.clone(), format!("{}/{}", gn, pc), ic.clone(), ml.clone(), *q, true, coeff)); }
    }}}
...
fn build_raw(ws: &mut rust_xlsxwriter::Worksheet, conn: &rusqlite::Connection, start: Option<&str>, end: Option<&str>, group_id: Option<i64>) -> Result<()> {
    ...
    let mut sql = String::from("SELECT pg.name AS gn, p.name AS pn, wr.user_name, wr.quantity, wr.recorded_at, p.coefficient FROM work_records wr JOIN projects p ON wr.project_id=p.id JOIN project_groups pg ON p.group_id=pg.id WHERE wr.deleted_at IS NULL AND wr.recorded_at>=?1 AND wr.recorded_at<=?2");
    ...
    let mut stmt = conn.prepare(&sql)?;
    let ra: Vec<(String, String, String, i64, String, f64)> = stmt.query_map(rusqlite::params_from_iter(params.iter().map(|p| p.as_ref())),
        |row| Ok((row.get(0)?, row.get(1)?, row.get(2)?, row.get(3)?, row.get(4)?, row.get::<_, f64>(5).unwrap_or(1.0))))?.
        collect::<std::result::Result<Vec<_>, _>>()?;
    let hdrs = ["序号","日期","实验室","项目名称","仪器","检测方法","仪器类型","数量","录入人","管理分值"];
    let cw = [6.0,12.0,14.0,30.0,12.0,30.0,8.0,8.0,12.0,10.0];
    for (i,h) in hdrs.iter().enumerate() { ws.write_with_format(0,i as u16,*h,&fmt.fh)?; ws.set_column_width(i as u16,cw[i])?; }
    for (i,(gn,pn,un,qty,ts,coeff)) in ra.iter().enumerate() {
        let ri = (i+1) as u32;
        let (_,ic,it) = export_data::parse_instrument(pn);
        let rd = &ts[..ts.len().min(10)];
        let score = (*qty as f64) * coeff;
        ws.write_with_format(ri,0,(i+1).to_string().as_str(),&fmt.fd)?;
        ws.write_with_format(ri,1,rd,&fmt.fd)?; ws.write_with_format(ri,2,gn.as_str(),&fmt.fd)?;
        ws.write_with_format(ri,3,pn.as_str(),&fmt.fd)?; ws.write_with_format(ri,4,ic,&fmt.fd)?;
        ws.write_with_format(ri,5,export_data::get_method_full_name(gn,pn).as_str(),&fmt.fd)?;
        ws.write_with_format(ri,6,it,&fmt.fd)?; ws.write_with_format(ri,7,qty.to_string().as_str(),&fmt.fd)?;
        ws.write_with_format(ri,8,un.as_str(),&fmt.fd)?;
        ws.write_with_format(ri,9,score,&fmt.fd)?;
    }
...
fn build_user_stats(ws: &mut rust_xlsxwriter::Worksheet, conn: &rusqlite::Connection, start: Option<&str>, end: Option<&str>, group_id: Option<i64>) -> Result<()> {
    ...
    for un in &user_order { for gn in user_data[un].keys() { for (pc, pr) in &user_data[un][gn] {
        let coeff = pr.coeff;
        for (ic,ml,q) in &pr.lc { rows.push((un.clone(),format!("{}/{}",gn,pc),ic.clone(),ml.clone(),*q,false,coeff)); }
        for (ic,ml,q) in &pr.gc { rows.push((un.clone(),format!("{}/{}",gn,pc),ic.clone(),ml.clone(),*q,true,coeff)); }
    }}}
