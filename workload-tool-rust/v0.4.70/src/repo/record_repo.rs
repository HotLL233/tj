...
pub fn create(pool: &DbPool, body: &RecordCreate) -> Result<RecordResponse> {
    ...
    tx.execute(
        "INSERT INTO work_records (project_id, method_id, user_name, quantity, recorded_at, group_id, division_id, multiplier, high_item)
         VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, COALESCE(?8, (SELECT m.multiplier FROM methods m WHERE m.id = ?2), 1.0), COALESCE(?9, (SELECT high_item FROM projects WHERE id=?1)))",
        rusqlite::params!(body.project_id, body.method_id, &body.user_name, body.quantity, &body.recorded_at, body.group_id, body.division_id, body.multiplier, body.high_item),
    )?;
