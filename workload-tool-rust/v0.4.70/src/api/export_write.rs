...
pub fn write_sheet1(
    ...
    ws.set_column_width(CJ, 12.0)?;  // 高项
    ...
    // 写表头
    let headers = ["使用实验室", "项目代号", "液相仪器", "检测方法", "单价倍率", "检测数量", "液相检测量", "气相检测量", "项目检测总量", "高项"];
    ...
    for (lab, proj, inst, method, multiplier, qty, _, _, high_item) in rows {
        ...
        ws.write_with_format(row_idx, CJ, high_item.as_deref().unwrap_or("-"), &fmt.fd)?;
        row_idx += 1;
    }
...
pub fn write_sheet2(
    ...
    ws.set_column_width(CG, 10.0)?;  // 单价倍率
    ws.set_column_width(CH, 12.0)?;  // 数量
    ws.set_column_width(CI, 15.0)?;  // 按天数量总计

    for col in 0u16..=8u16 {
        ws.set_column_format(col, &fmt.fd)?;
    }

    let headers = ["日期", "仪器", "实验室", "项目", "高项", "方法", "单价倍率", "数量", "按天数量总计"];
    for (i, h) in headers.iter().enumerate() {
        ws.write_with_format(HR, i as u16, *h, &fmt.fh)?;
    }
    ...
    for row in rows {
        ...
        ws.write_with_format(row_idx, CG, row.multiplier, &fmt.fd)?;
        ...
        if !current_date.is_empty() && current_date != row.date {
            ...
            ws.write_formula(date_start, CI, format!("=SUM({}{}:{}{})", col_letter(CH), date_start + 1, col_letter(CH), sum_row + 1).as_str())?;
            date_start = row_idx;
        }
    ...
    if row_idx > HR + 1 {
        ws.write_formula(date_start, CI, format!("=SUM({}{}:{}{})", col_letter(CH), date_start + 1, col_letter(CH), row_idx).as_str())?;
    }
...
pub fn write_sheet3(
    ...
    let headers = ["项目", "高项", "实验室", "仪器", "方法", "单价倍率", "数量", "单价", "金额总计", "项目金额"];
    for (i, h) in headers.iter().enumerate() {
        ws.write_with_format(HR, i as u16, *h, &fmt.fh)?;
    }
...
pub fn write_sheet4(
    ...
    let headers = ["实验室", "项目", "高项", "仪器", "方法", "单价倍率", "数量", "单价", "数量总计", "金额总计", "实验室汇总"];
    for (i, h) in headers.iter().enumerate() {
        ws.write_with_format(HR, i as u16, *h, &fmt.fh)?;
    }
...
pub fn write_sheet5(
    ...
    ws.set_column_width(CG, 10.0)?;  // 单价倍率
    ws.set_column_width(CH, 10.0)?;  // 数量
    ws.set_column_width(CI, 12.0)?;  // 录入人

    for col in 0u16..=8u16 {
        ws.set_column_format(col, &fmt.fd)?;
    }

    let headers = ["录入时间", "实验室", "研发项目", "高项", "方法", "检测类型", "单价倍率", "数量", person_label];
    for (i, h) in headers.iter().enumerate() {
        ws.write_with_format(HR, i as u16, *h, &fmt.fh)?;
    }
    ...
    for row in rows {
        ...
        ws.write_with_format(row_idx, CG, row.multiplier, &fmt.fd)?;
        ws.write_with_format(row_idx, CH, row.quantity as f64, &fmt.fd)?;
        ws.write_with_format(row_idx, CI, row.user_name.as_str(), &fmt.fd)?;
        row_idx += 1;
    }
