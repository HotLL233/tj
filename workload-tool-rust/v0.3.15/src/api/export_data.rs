...
pub fn query_sheet1_data(
    ...
    // v0.3.14 修复：改用 JOIN project_lab_links 替代 LIMIT 1 子查询
    // 原来的标量子查询 LIMIT 1 导致多实验室项目永远只显示第一个实验室
    let mut sql = String::from(
        "SELECT COALESCE(pg.name, '未知') as lab_name,
         ...
         LEFT JOIN project_lab_links pll ON pll.project_id = p.id
    ...
    let sql =
        "SELECT date(wr.recorded_at) AS record_date,
         ...
         WHERE wr.deleted_at IS NULL
           AND wr.recorded_at >= ?1
           AND wr.recorded_at <= ?2
         GROUP BY record_date, COALESCE(pg.name, '未知'), m.id, p.id
    ...
    let sql =
        "SELECT p.name AS project_name,
         ...
         WHERE wr.deleted_at IS NULL
           AND wr.recorded_at >= ?1
           AND wr.recorded_at <= ?2
         GROUP BY COALESCE(pg.name, '未知'), p.id, m.id
    ...
    let sql =
        "SELECT COALESCE(pg.name, '未知') AS lab_name,
         ...
         WHERE wr.deleted_at IS NULL
           AND wr.recorded_at >= ?1
           AND wr.recorded_at <= ?2
         GROUP BY COALESCE(pg.name, '未知'), p.id, mt.name, m.amount
