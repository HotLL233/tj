...
pub fn row_to_project(row: &rusqlite::Row) -> rusqlite::Result<ProjectResponse> {
    Ok(ProjectResponse {
        ...
        coefficient: row.get::<_, f64>(8).unwrap_or(1.0), method_type: row.get(9)?,
        associated_lab_ids: row.get(11)?, associated_method_ids: row.get(12)?, parent_id: 0,
...
const PROJ_SQL: &str =
    "SELECT p.id, p.group_id, pg.name, p.name, COALESCE(p.full_name,''), COALESCE(p.notes,''), \
     ...
     COALESCE(p.associated_lab_ids,''), COALESCE(p.associated_method_ids,'') \
...
pub fn create(pool: &DbPool, body: &ProjectCreate) -> Result<ProjectResponse> {
    ...
    let ali = body.associated_lab_ids.as_deref().unwrap_or("");
    let ami = body.associated_method_ids.as_deref().unwrap_or("");
    conn.execute("INSERT INTO projects (group_id, name, sort_order, coefficient, method_type, associated_lab_ids, associated_method_ids) VALUES (?1,?2,?3,?4,?5,?6,?7)",
        rusqlite::params![body.group_id, body.name, so, cf, mt, ali, ami])?;
...
pub fn update(pool: &DbPool, id: i64, body: &ProjectUpdate) -> Result<ProjectResponse> {
    ...
    if let Some(ref ids) = body.associated_lab_ids { conn.execute("UPDATE projects SET associated_lab_ids=?1 WHERE id=?2", (ids, id))?; }
    if let Some(ref ids) = body.associated_method_ids { conn.execute("UPDATE projects SET associated_method_ids=?1 WHERE id=?2", (ids, id))?; }
    get_by_id(pool, id)
}
