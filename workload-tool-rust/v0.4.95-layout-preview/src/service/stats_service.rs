...
/// 未分配事业部的记录归入 "N/A"
pub fn by_division(pool: &DbPool, start: &str, end: &str, division_id: Option<i64>, user_name: Option<&str>) -> Result<Vec<DivisionSummary>> {
    let conn = pool.get()?;
    let end_closed = format!("{}T23:59:59", end);
    ...
    if let Some(user) = user_name {
        let idx = params.len() + 1;
        clauses.push(format!("wr.user_name = ?{}", idx));
    ...
    let sql = format!(
        "SELECT d.id, COALESCE(d.name, 'N/A') AS division_name,
                ...
                COALESCE(SUM(wr.quantity * wr.coefficient_snapshot), 0.0) AS coefficient_score,
