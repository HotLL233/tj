...
pub fn list_by_records(
    ...
    let mut stmt = conn.prepare(&format!(
        "SELECT id, record_id, file_name, stored_name, file_size, file_type, created_at
         FROM sample_info_attachments WHERE record_id IN ({}) ORDER BY created_at DESC",
        placeholders
    ))?;
    let rows = stmt.query_map(rusqlite::params_from_iter(record_ids.iter()), |row| {
        Ok(SampleInfoAttachment {
            id: row.get(0)?,
            record_id: row.get(1)?,
            file_name: row.get(2)?,
    ...
    for attachment in rows {
        ...
        result.entry(attachment.record_id).or_default().push(attachment);
    }
    Ok(result)
}
