...
async fn create(State(pool): State<DbPool>, headers: HeaderMap, Json(body): Json<RdRecordCreate>) -> Result<Json<ApiResponse<RdRecordResponse>>> {
    ...
    let result = rd_record_service::create_record(&pool, &record, body.batch_no, body.notes, &user_name)?;
...
async fn restore(State(pool): State<DbPool>, headers: HeaderMap, Path(id): Path<i64>) -> Result<Json<ApiResponse<RdRecordResponse>>> {
    Ok(Json(ApiResponse::ok(rd_record_repo::restore(&pool, id, &extract_claims_username(&headers))?)))
}
...
async fn delete_by_user(
    ...
    let count = rd_record_repo::delete_by_user(&pool, &user_name, q.start.as_deref(), q.end.as_deref(), &extract_claims_username(&headers))?;
