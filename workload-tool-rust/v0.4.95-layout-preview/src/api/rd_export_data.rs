...
pub fn query_sheet1_data(
    ...
    let rows = stmt.query_map(
        ...
        |row| {
            ...
            Ok((lab, project_code, instrument, method, 1.0, quantity, is_gc, coefficient, None))
        }
    )?;
    ...
    fn rd_amount_sheets_use_method_multiplier_and_type_sheet_does_not() {
        ...
        conn.execute_batch(
            "CREATE TABLE project_groups (id INTEGER PRIMARY KEY, name TEXT);
             ...
             INSERT INTO methods VALUES (1, 'Method01@[LC-01]', 'Method01', 10.0, 2.5);
             INSERT INTO method_types VALUES (1, 'Type01');
             INSERT INTO method_type_links VALUES (1, 1);
             INSERT INTO rd_work_records VALUES (1, 1, 1, 3, '2026-07-17T10:00:00', 1, NULL);"
        ).unwrap();

        assert_eq!(query_sheet3_data(&conn, "2026-07-17", "2026-07-17").unwrap()[0].multiplier, 2.5);
        assert_eq!(query_sheet4_data(&conn, "2026-07-17", "2026-07-17").unwrap()[0].multiplier, 2.5);
        assert_eq!(query_sheet7_data(&conn, "2026-07-17", "2026-07-17").unwrap()[0].multiplier, 2.5);
        assert_eq!(query_sheet8_data(&conn, "2026-07-17", "2026-07-17").unwrap()[0].multiplier, 2.5);
        assert_eq!(query_sheet11_data(&conn, "2026-07-17", "2026-07-17").unwrap()[0].multiplier, 1.0);
    }
}
