  ...
  const renderRecordCell = useCallback((rec: WorkRecord, field: FieldDef, idx: number): React.ReactNode => {
    ...
    switch (field.key) {
      ...
      case 'lab_name':
        ...
        return (
          <TableCell key={field.key} sx={{ fontSize: '0.8rem', minWidth: Math.max(field.width || 150, 150), whiteSpace: 'normal', wordBreak: 'break-word' }}>
      ...
      case 'method_name':
        return (
          <TableCell key={field.key} sx={{ fontSize: '0.8rem', minWidth: Math.max(field.width || 220, 220), whiteSpace: 'normal', wordBreak: 'break-word' }}>
            {rec.method_name || '-'}
          </TableCell>
        );
      case 'detection_type':
      ...
      case 'notes':
        return (
          <TableCell key={field.key} sx={{ fontSize: '0.8rem', minWidth: Math.max(field.width || 180, 180), whiteSpace: 'normal', wordBreak: 'break-word' }}>
            {rec.notes || '-'}
          </TableCell>
        );
      default:
