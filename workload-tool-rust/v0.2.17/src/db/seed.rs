...
pub fn ensure_seeded(conn: &rusqlite::Connection) -> Result<()> {
    ...
    // v0.2.17: 种子数据适配新表 — projects 仅存研发项目，group link 通过 project_lab_links
    for (gi, (group_name, project_names)) in DEFAULT_DATA.iter().enumerate() {
        conn.execute(
            "INSERT INTO project_groups (name, sort_order) VALUES (?1, ?2)",
            (group_name, gi as i64),
        )?;
        let group_id = conn.last_insert_rowid();
        for (pi, proj_name) in project_names.iter().enumerate() {
            conn.execute(
                "INSERT INTO projects (group_id, name, sort_order, method_type) VALUES (?1, ?2, ?3, '研发项目')",
                rusqlite::params!(group_id, *proj_name, pi as i64),
            )?;
            let project_id = conn.last_insert_rowid();
            // Link project → lab via project_lab_links
            conn.execute(
