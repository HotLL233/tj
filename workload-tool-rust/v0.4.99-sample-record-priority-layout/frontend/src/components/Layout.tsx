    ...
    <Dialog open={aboutOpen} onClose={() => setAboutOpen(false)} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: '2px' } }}>
      ...
      <DialogContent>
        ...
        <Alert severity="info" sx={{ mb: 2, borderRadius: '2px' }}>
          <Typography variant="subtitle2" sx={{ fontWeight: 600, mb: 0.5 }}>v0.4.99 样品记录核心信息与附件优化版</Typography>
          <Typography variant="body2">样品记录优先显示批号、数量、主要成分和检测元素。</Typography>
          <Typography variant="body2">附件列扩大，附件名称直接完整换行显示。</Typography>
          <Typography variant="body2">桌面端重新压缩短字段列宽，手机卡片同步调整信息顺序。</Typography>
        </Alert>
