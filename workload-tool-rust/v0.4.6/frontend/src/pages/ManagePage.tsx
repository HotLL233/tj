              ...
              renderView={(item) => (
                <Box>
                  ...
                  <Typography variant="caption" color="text.secondary">
                    ...
                    倍率: <Chip label={(item.multiplier ?? 1.0).toFixed(1)} size="small" variant="outlined" color="secondary" sx={{ borderRadius: R, height: 20, fontSize: '0.7rem', ml: 0.5 }} />
    ...
    <Dialog open={methodEditOpen} onClose={() => { setMethodEditOpen(false); setMethodEditItem(null); }} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: R } }}>
      ...
      <DialogContent>
        {methodEditItem && <>
          ...
          <TextField label="倍率" type="number" fullWidth value={methodEditItem.multiplier ?? 1.0} onChange={e => setMethodEditItem({ ...methodEditItem, multiplier: Number(e.target.value) || 1.0 })} sx={{ mb: 2, '& .MuiOutlinedInput-root': { borderRadius: R } }} inputProps={{ min: 0, step: 0.1 }} />
          ...
          <Table size="small" stickyHeader>
            <TableHead>
              <TableRow>
                ...
                <TableCell sx={{ fontWeight: 700, minWidth: 160 }}>类型</TableCell>
                <TableCell sx={{ fontWeight: 700 }}>系数</TableCell>
                <TableCell sx={{ fontWeight: 700 }}>倍率</TableCell>
