...
pub fn list(
    ...
    if let Some(gid) = group_id { where_clauses.push(format!("pg.id={}", gid)); }
    ...
    let count: i64 = conn.query_row(
        &format!("SELECT COUNT(*) FROM work_records wr JOIN projects p ON wr.project_id = p.id JOIN project_groups pg ON p.group_id = pg.id {}", where_sql),
    ...
    let sql = format!(
        "SELECT wr.id, wr.project_id, p.name, pg.name, wr.user_name, wr.quantity,
                ...
                (SELECT m.name FROM project_method_links pml
                 JOIN methods m ON pml.method_id = m.id
                 WHERE pml.project_id = wr.project_id LIMIT 1) AS method_name,
                (SELECT group_concat(DISTINCT mt.name)
                 FROM project_method_links pml
                 JOIN methods m ON pml.method_id = m.id
                 JOIN method_type_links mtl ON m.id = mtl.method_id
                 JOIN method_types mt ON mtl.method_type_id = mt.id
                 WHERE pml.project_id = wr.project_id) AS method_type
    ...
    let rows = stmt.query_map(
        ...
        |row| Ok(RecordResponse {
            ...
            method_name: row.get(9)?, method_type: row.get(10)?,
        }),
    )?;
