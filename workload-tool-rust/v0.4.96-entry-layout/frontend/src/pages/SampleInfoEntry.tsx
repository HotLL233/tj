        ...
        <Box sx={{ display: 'grid', gap: 1.25, mb: 2 }}>
          {rows.map((row, idx) => (
            <Paper key={idx} variant="outlined" sx={{ p: { xs: 1, md: 1.5 }, borderRadius: '6px', bgcolor: row.checked ? '#f1f8f2' : '#fff' }}>
              ...
              <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2,minmax(0,1fr))', lg: 'repeat(4,minmax(0,1fr))', xl: 'repeat(5,minmax(0,1fr))' }, gap: 1.25, alignItems: 'start' }}>
                {formColumns.map(col => (
                  <Box key={col.field_key} sx={{ minWidth: 0 }}>
