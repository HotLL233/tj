...
const SampleEntryPage: React.FC = () => {
  ...
  const recordAdaptiveWidths = useMemo(() => {
    return getAdaptiveColumnWidths(todayRecords, [
      ...
      { key: '_high_item', header: '高项', min: 58, max: 90, getValue: (rec: WorkRecord) => (rec as any).high_item || '-' },
    ]);
          ...
          {rows.map((row, idx) => {
            ...
            return (
              <Paper key={row.id} variant="outlined" sx={{ borderRadius: R, boxShadow: 'none', overflow: 'hidden' }}>
                <Box sx={{
                  display: 'grid',
                  gridTemplateColumns: { xs: '42px 48px repeat(2, minmax(0, 1fr))', md: '42px 54px repeat(5, minmax(0, 1fr))' },
                  alignItems: 'center',
                  gap: 1,
                  px: 1,
                  py: 0.75,
              ...
              <TableBody>
                {todayRecords.map((rec, idx) => (
                  <TableRow key={rec.id} hover
                    ...
                    <TableCell sx={{ ...rdRecordCellSx, ...adaptiveCellSx(recordAdaptiveWidths._high_item) }}>{(rec as any).high_item || '-'}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
