    ...
    <Dialog open={aboutOpen} onClose={() => setAboutOpen(false)} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: '2px' } }}>
      ...
      <DialogContent>
        ...
        <Alert severity="info" sx={{ mb: 2, borderRadius: '2px' }}>
          <Typography variant="subtitle2" sx={{ fontWeight: 600, mb: 0.5 }}>v0.4.96 登记与记录布局优化版</Typography>
          <Typography variant="body2">样品信息登记改为宽屏多列、窄屏自动换行的录入布局。</Typography>
          <Typography variant="body2">记录区使用 PC 组合列和手机卡片，长文本完整换行显示。</Typography>
          <Typography variant="body2">记录编辑直接显示在当前记录内，不再依赖折叠详情。</Typography>
          <Typography variant="body2">样品信息统计页同步扩大数据工作区，减少宽屏留白。</Typography>
        </Alert>
