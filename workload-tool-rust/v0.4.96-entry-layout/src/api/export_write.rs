...
const SHEET3_HEADERS: [&str; 10] = [
    "项目", "高项", "实验室", "仪器", "方法", "单价倍率", "数量", "单价", "明细金额", "项目金额汇总",
];
const SHEET4_HEADERS: [&str; 11] = [
    "实验室", "项目", "高项", "仪器", "方法", "单价倍率", "数量", "单价", "数量总计", "明细金额", "实验室金额汇总",
];
const SHEET6_HEADERS: [&str; 12] = [
    "人员", "液相数量", "液相系数", "液相工作量", "气相数量", "气相系数",
    "气相工作量", "理化数量", "理化系数", "理化工作量", "个人工作量汇总", "总工作量",
];
const SHEET7_HEADERS: [&str; 16] = [
    "实验室", "项目", "液相数量", "液相单价", "液相单价倍率", "液相金额汇总",
    "气相数量", "气相单价", "气相单价倍率", "气相金额汇总",
    "理化数量", "理化单价", "理化单价倍率", "理化金额汇总", "项目金额汇总", "实验室金额汇总",
];
const SHEET8_HEADERS: [&str; 14] = [
    "项目", "液相数量", "液相单价", "液相单价倍率", "液相金额汇总",
    "气相数量", "气相单价", "气相单价倍率", "气相金额汇总",
    "理化数量", "理化单价", "理化单价倍率", "理化金额汇总", "项目金额汇总",
];
const SHEET11_HEADERS: [&str; 5] = [
    "检测类型", "数量", "单价", "明细金额", "检测类型金额汇总",
];
...
pub fn write_sheet1(
    ...
    // 写表头
    let headers = ["使用实验室", "项目代号", "液相仪器", "检测方法", "检测数量", "液相检测量", "气相检测量", "项目检测总量", "高项"];
...
pub fn write_sheet2(
    ...
    let headers = ["日期", "仪器", "实验室", "项目", "高项", "方法", "数量", "按天数量总计"];
    for (i, h) in headers.iter().enumerate() {
        ws.write_with_format(HR, i as u16, *h, &fmt.fh)?;
    }
    ...
    for row in rows {
        ...
        ws.write_with_format(row_idx, CE, row.high_item.as_deref().unwrap_or("-"), &fmt.fd)?;
...
pub fn write_sheet3(
    ...
    for row in rows {
        ...
        ws.write_with_format(row_idx, CB, row.high_item.as_deref().unwrap_or("-"), &fmt.fd)?;
        ws.write_with_format(row_idx, CC, row.lab.as_str(), &fmt.fd)?;
        ...
        // 金额总计 = 数量 × 单价 × 单价倍率
        ws.write_formula(row_idx, CI, format!("={}{}*{}{}*{}{}", col_letter(CG), row_idx + 1, col_letter(CH), row_idx + 1, col_letter(CF), row_idx + 1).as_str())?;
...
pub fn write_sheet4(
    ...
    for row in rows {
        ...
        ws.write_with_format(row_idx, CC, row.high_item.as_deref().unwrap_or("-"), &fmt.fd)?;
        ws.write_with_format(row_idx, CD, row.instrument.as_str(), &fmt.fd)?;
        ws.write_with_format(row_idx, CE, row.method.as_str(), &fmt.fd)?;
        ws.write_with_format(row_idx, CF, row.multiplier, &fmt.fd)?;
        ws.write_with_format(row_idx, CG, row.quantity as f64, &fmt.fd)?;
        ...
        // 数量总计 = 数量
        ws.write_formula(row_idx, CI, format!("={}{}", col_letter(CG), row_idx + 1).as_str())?;
        // 金额总计 = 数量总计 × 单价 × 单价倍率
        ws.write_formula(row_idx, CJ, format!("={}{}*{}{}*{}{}", col_letter(CI), row_idx + 1, col_letter(CH), row_idx + 1, col_letter(CF), row_idx + 1).as_str())?;
    ...
    for &(start, end, ref lab_name) in &lab_groups {
        ...
        ws.write_formula(start, CK, format!("=SUM({}{}:{}{})", col_letter(CJ), start + 1, col_letter(CJ), end + 1).as_str())?;
    }
...
pub fn write_sheet5(
    ...
    let headers = ["录入时间", "实验室", "研发项目", "高项", "方法", "检测类型", "数量", person_label];
    for (i, h) in headers.iter().enumerate() {
        ws.write_with_format(HR, i as u16, *h, &fmt.fh)?;
    }
    ...
    for row in rows {
        ...
        ws.write_with_format(row_idx, CD, row.high_item.as_deref().unwrap_or("-"), &fmt.fd)?;
        ws.write_with_format(row_idx, CE, row.method.as_str(), &fmt.fd)?;
...
pub fn write_sheet6(
    ...
    let headers = [
        person_label,
        SHEET6_HEADERS[1], SHEET6_HEADERS[2], SHEET6_HEADERS[3], SHEET6_HEADERS[4], SHEET6_HEADERS[5],
        SHEET6_HEADERS[6], SHEET6_HEADERS[7], SHEET6_HEADERS[8], SHEET6_HEADERS[9], SHEET6_HEADERS[10], SHEET6_HEADERS[11],
    ];
    for (i, h) in headers.iter().enumerate() {
        ws.write_with_format(HR, i as u16, *h, &fmt.fh)?;
    }
    ...
    // 总额
    if row_idx > HR + 1 {
        ws.write_formula(HR + 1, CL, format!("=SUM({}{}:{}{})", col_letter(CK), HR+2, col_letter(CK), row_idx).as_str())?;
    }
...
pub fn write_sheet11(
    ...
    for &(start, end, ref type_name) in &type_groups {
        ...
        ws.write_formula(start, CE, format!("=SUM({}{}:{}{})", col_letter(CD), start+1, col_letter(CD), end+1).as_str())?;
    }
    ...
    // 金额总计行合计
    ws.write_formula(total_row, CD, format!("=SUM({}{}:{}{})", col_letter(CD), HR+2, col_letter(CD), total_row).as_str())?;
