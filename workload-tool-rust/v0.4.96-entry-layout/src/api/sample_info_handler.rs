...
fn ensure_status_permission(headers: &HeaderMap, target_status: &str) -> Result<String> {
    let claims = extract_claims(headers).ok_or_else(|| AppError::Forbidden("未登录".into()))?;
    ...
    if !claims.is_admin && !claims.permissions.iter().any(|p| p == "*" || p == required) {
...
pub fn router(pool: DbPool) -> Router {
    Router::new()
        ...
        .route("/api/sample-info/:id/complete", axum::routing::put(complete))
        .with_state(pool)
}
...
async fn create(
    ...
    let record = sample_info_repo::create(&pool, &body, &user_name)?;
...
async fn update_status(
    ...
    let user_name = ensure_status_permission(&headers, &body.status)?;
...
async fn sample(
    ...
    let user_name = ensure_status_permission(&headers, "待检测")?;
    Ok(Json(ApiResponse::ok(sample_info_repo::update_status(
        &pool, id, "待检测", &user_name,
    )?)))
}
...
async fn complete(
    ...
    let user_name = ensure_status_permission(&headers, "已检测")?;
    Ok(Json(ApiResponse::ok(sample_info_repo::update_status(
        &pool, id, "已检测", &user_name,
    )?)))
}
