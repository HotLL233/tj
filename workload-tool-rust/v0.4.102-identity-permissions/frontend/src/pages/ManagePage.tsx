...
import { getGroups, createGroup, updateGroup, deleteGroup, getProjects, createProject, updateProject, deleteProject, getRecords, restoreRecord, getRdRecords, restoreRdRecord, getAuditLogs, getRecordTrace, batchProjectCoefficient, getBackupStatus, backupNow, getBackupConfig, updateBackupConfig, deleteBackup, restoreBackup, restoreBackupFile, getMethodTypes, createMethodType, updateMethodType, deleteMethodType, getMethods, createMethod, updateMethod, deleteMethod, methodImport, getImportMappings, getInstruments, createInstrument, updateInstrument, deleteInstrument, getHelpDocuments, uploadHelpDocument, updateHelpDocument, deleteHelpDocument, getHelpArticles, deleteHelpArticle, updateHelpArticle, getSampleInfoTypesAll, getSampleInfoRecords, restoreSampleInfo, updateSampleInfo, deleteSampleInfo, getSampleInfoTypes, getSampleInfoStats, createSampleInfoType, updateSampleInfoType, deleteSampleInfoType, deleteSampleInfoTypePermanent, exportSampleInfo, getDivisions, createDivision, updateDivision, deleteDivision, setDivisionLabs, getSampleInfoColumns, createSampleInfoColumn, updateSampleInfoColumn, deleteSampleInfoColumn, reorderSampleInfoColumns, updateSampleInfoColumnTypes, userList, createUser, updateUser, deleteUser, deleteUserPermanent, getRoles, updateSetting, importUsers, reorderHelpDocuments, reorderHelpArticles } from '../api/client';
import type { ProjectGroup, Project, WorkRecord, AuditLog, RecordEvent, BackupStatus, MethodType, Method, Instrument, ImportMapping, HelpDocument, HelpArticle, SampleInfoType, SampleInfoRecord, Division, SampleInfoColumn, User, UserUpdate, Role, RoleWithPermissions, SystemSetting, HomeCard, StatCard, ManageTab } from '../types';
...
type TV = 'projects' | 'groups' | 'methods' | 'divisions' | 'master-import' | 'trash' | 'audit' | 'backup' | 'help' | 'sampleinfo' | 'users' | 'roles' | 'layouts' | 'theme' | 'home' | 'stats' | 'forms' | 'exports' | 'sessions';

const ROUTABLE_TABS: TV[] = ['projects', 'groups', 'divisions', 'master-import', 'methods', 'trash', 'audit', 'backup', 'help', 'sampleinfo', 'users', 'roles', 'layouts', 'forms', 'exports', 'stats', 'sessions'];
...
const ManagePage: React.FC = () => {
  ...
  const ongoingProjects = useMemo(() => ps.filter(p => (p.project_status || 'ongoing') !== 'archived'), [ps]);
  const archivedProjects = useMemo(() => ps.filter(p => (p.project_status || 'ongoing') === 'archived'), [ps]);
  ...
  const defaultTC: { key: TV; label: string; perm?: string; icon: React.ReactNode; desc: string }[] = [
    ...
    { key: 'stats', label: '统计管理', perm: 'stats:workload:access', icon: <AssessmentIcon />, desc: '统计入口、卡片和查看权限' },
  ];
  ...
  const [tr, setTr] = useState<Array<WorkRecord & { trash_source: 'work' | 'rd' | 'sample-info' }>>([]);
  const [trTypes, setTrTypes] = useState<SampleInfoType[]>([]);
  const [trProjects, setTrProjects] = useState<any[]>([]);
  const [trUsers, setTrUsers] = useState<User[]>([]);
  const [trashTab, setTrashTab] = useState<'records' | 'types' | 'projects' | 'users'>('records');
  const loadTrash = async () => {
    const records: Array<any> = [];
    if (user?.is_admin || hasPermission(user?.permissions || [], 'entry:workload')) {
      try { const r = await getRecords({ include_deleted: true, page_size: 500 }); if (r.code === 0 && r.data) records.push(...r.data.items.filter((x: any) => x.deleted_at != null).map((x: any) => ({ ...x, trash_source: 'work' }))); } catch {}
    }
    if (user?.is_admin || ['entry:sample', 'records:rd:view-all', 'records:rd:view-lab'].some(p => hasPermission(user?.permissions || [], p))) {
      try { const r = await getRdRecords({ include_deleted: true, page_size: 500 }); if (r.code === 0 && r.data) records.push(...r.data.items.filter((x: any) => x.deleted_at != null).map((x: any) => ({ ...x, trash_source: 'rd' }))); } catch {}
    }
    if (user?.is_admin || hasPermission(user?.permissions || [], 'entry:sample-info')) {
      try { const r = await getSampleInfoRecords({ include_deleted: true, page_size: 500 }); if (r.code === 0 && r.data) records.push(...r.data.items.filter(x => x.deleted_at != null).map(x => ({ ...x, recorded_at: x.submitted_at, trash_source: 'sample-info' }))); } catch {}
    }
    setTr(records.sort((a, b) => String(b.deleted_at || '').localeCompare(String(a.deleted_at || ''))));
    if (user?.is_admin || hasPermission(user?.permissions || [], 'manage:sampleinfo')) {
      try { const r = await getSampleInfoTypesAll(); if (r.code === 0 && r.data) setTrTypes(r.data.filter((t: any) => !t.is_active)); } catch {}
    } else setTrTypes([]);
    if (user?.is_admin || hasPermission(user?.permissions || [], 'manage:projects')) {
      try { const r = await getProjects(); if (r.code === 0 && r.data) setTrProjects(r.data.filter((p: any) => !p.is_active)); } catch {}
    } else setTrProjects([]);
    if (user?.is_admin || hasPermission(user?.permissions || [], 'manage:users')) {
      try { const r = await userList(); if (r.code === 0 && r.data) setTrUsers(r.data.filter((u: any) => !u.is_active)); } catch {}
    } else setTrUsers([]);
  };
  // v0.4.63: 导入用户对话框
  ...
  const trashRecordWidths = useMemo(() => getAdaptiveColumnWidths(tr, [
    { key: 'trash_source', header: '来源', fixed: 86, getValue: r => r.trash_source },
    { key: 'recorded_at', header: '日期', fixed: 118, getValue: r => r.recorded_at },
    { key: 'project_name', header: '项目', min: 78, max: 160, getValue: r => r.project_name },
    { key: 'user_name', header: '用户', min: 62, max: 110, getValue: r => r.user_name },
    { key: 'quantity', header: '数量', fixed: 58, getValue: r => r.quantity },
  ...
  useEffect(() => { setLd(true); Promise.all([lg(), lp(), lt(), lm(), li(), lmt()]).finally(() => setLd(false)); }, [lg, lp, lt, lm, li, lmt]);
  useEffect(() => { if (tb === 'audit') la(1); if (tb === 'backup') loadBk(); if (tb === 'methods') { lmt(); lm(); li(); } if (tb === 'trash') loadTrash(); if (tb === 'help') { loadHelpDocs(); loadHelpArticles(); } if (tb === 'users') { userList().then(r => { if (r.code === 0 && r.data) setUsers(r.data.filter((u: any) => u.is_active)); }).catch(() => {}); getRoles().then(r => { if (r.code === 0 && r.data) setRoles(r.data); }).catch(() => {}); } }, [tb, la, lmt, lm, li, loadUsers]);
  ...
  return (
    ...
    {showSectionContent && tb === 'groups' && <Box>
      ...
      {gs.length === 0 ? <Typography color="text.secondary" textAlign="center" sx={{ py: 4 }}>暂无实验室分组</Typography>
        : <Paper variant="outlined" elevation={0} sx={{ borderRadius: R, overflow: 'hidden' }}>
          {gs.map((g, index) => (
            <Box key={g.id} sx={{ display: 'grid', gridTemplateColumns: { xs: 'minmax(0,1fr) auto', md: 'minmax(0,1fr) 120px 100px' }, alignItems: 'center', gap: 1.5, px: { xs: 1.25, md: 2 }, py: 1.1, borderBottom: index === gs.length - 1 ? 'none' : '1px solid #edf0f2' }}>
              <Box sx={{ minWidth: 0 }}>
              ...
              <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 0.25 }}>
                ...
                <IconButton size="small" color="error" onClick={() => { setCa(() => async () => { const r = await deleteGroup(g.id); if (r.code === 0) { sm('删除成功'); lg(); } else sm(r.message, true); setCo(false); }); setCo(true); }} aria-label={`删除${g.name}`}><DeleteIcon fontSize="small" /></IconButton>
              </Box>
            </Box>
          ))}
        </Paper>}
    ...
    {showSectionContent && tb === 'methods' && <Box>
      <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, justifyContent: 'space-between', alignItems: { xs: 'stretch', md: 'center' }, gap: 1.25, mb: 2 }}>
        <Box>
      ...
      {ml.filter(m => !methodFilter || m.type_names.includes(methodFilter)).length === 0 ? <Typography color="text.secondary" textAlign="center" sx={{ py: 4 }}>暂无检测方法数据，请先导入方法或手动创建</Typography>
        : <Paper variant="outlined" elevation={0} sx={{ borderRadius: R, overflow: 'hidden' }}>
          {ml.filter(m => !methodFilter || m.type_names.includes(methodFilter)).map((m, index, filtered) => (
            <Box key={m.id} sx={{ display: 'grid', gridTemplateColumns: { xs: 'minmax(0,1fr) auto', md: 'minmax(260px,1.6fr) minmax(180px,1fr) minmax(180px,1fr) auto' }, alignItems: 'center', gap: 1.5, px: { xs: 1.25, md: 2 }, py: 1.1, borderBottom: index === filtered.length - 1 ? 'none' : '1px solid #edf0f2' }}>
              ...
              <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 0.25 }}>
                ...
                <IconButton size="small" color="error" onClick={() => { setCa(() => async () => { const r = await deleteMethod(m.id); if (r.code === 0) { sm('删除成功'); lm(); } else sm(r.message, true); setCo(false); }); setCo(true); }} aria-label={`删除${m.name}`}><DeleteIcon fontSize="small" /></IconButton>
              </Box>
            </Box>
          ))}
        </Paper>}
    ...
    {showSectionContent && tb === 'trash' && <Box>
      <Typography variant="body2" color="text.secondary" sx={{ mb: 1, display: 'flex', alignItems: 'center', gap: 1 }}><DeleteSweepIcon fontSize="small" />回收站</Typography>
      <Box sx={{ display: 'flex', gap: 1, mb: 2, flexWrap: 'wrap' }}>
        <Chip label={`工作记录 (${tr.length})`} size="small" onClick={() => setTrashTab('records')} color={trashTab === 'records' ? 'primary' : 'default'} variant={trashTab === 'records' ? 'filled' : 'outlined'} sx={{ borderRadius: R }} />
        {(user?.is_admin || hasPermission(user?.permissions || [], 'manage:sampleinfo')) && <Chip label={`检测类型 (${trTypes.length})`} size="small" onClick={() => setTrashTab('types')} color={trashTab === 'types' ? 'primary' : 'default'} variant={trashTab === 'types' ? 'filled' : 'outlined'} sx={{ borderRadius: R }} />}
        {(user?.is_admin || hasPermission(user?.permissions || [], 'manage:projects')) && <Chip label={`研发项目 (${trProjects.length})`} size="small" onClick={() => setTrashTab('projects')} color={trashTab === 'projects' ? 'primary' : 'default'} variant={trashTab === 'projects' ? 'filled' : 'outlined'} sx={{ borderRadius: R }} />}
        {(user?.is_admin || hasPermission(user?.permissions || [], 'manage:users')) && <Chip label={`用户 (${trUsers.length})`} size="small" onClick={() => setTrashTab('users')} color={trashTab === 'users' ? 'primary' : 'default'} variant={trashTab === 'users' ? 'filled' : 'outlined'} sx={{ borderRadius: R }} />}
      </Box>
      {trashTab === 'records' && (tr.length === 0 ? <Typography color="text.secondary" textAlign="center" sx={{ py: 4 }}>回收站为空</Typography>
        : <TableContainer component={Paper} className="table-responsive" sx={tSx}>
          <Table size="small" sx={adaptiveTableSx}>
            <TableHead><TableRow>
              <TableCell sx={{ ...adaptiveCellSx(trashRecordWidths.trash_source), fontWeight: 600 }}>来源</TableCell>
              <TableCell sx={{ ...adaptiveCellSx(trashRecordWidths.recorded_at), fontWeight: 600 }}>日期</TableCell>
              <TableCell sx={{ ...adaptiveCellSx(trashRecordWidths.project_name), fontWeight: 600 }}>项目</TableCell>
              <TableCell sx={{ ...adaptiveCellSx(trashRecordWidths.user_name), fontWeight: 600 }}>用户</TableCell>
              <TableCell align="right" sx={{ ...adaptiveCellSx(trashRecordWidths.quantity), fontWeight: 600 }}>数量</TableCell>
              <TableCell align="right" sx={{ ...adaptiveCellSx(trashRecordWidths.actions), fontWeight: 600 }}>操作</TableCell>
            </TableRow></TableHead>
            <TableBody>{tr.map(r => (
              <TableRow key={`${r.trash_source}-${r.id}`} hover>
                <TableCell sx={adaptiveCellSx(trashRecordWidths.trash_source)}>{{ work: '分析检测', rd: '研发送样', 'sample-info': '样品登记' }[r.trash_source]}</TableCell>
                <TableCell sx={adaptiveCellSx(trashRecordWidths.recorded_at)}>{r.recorded_at}</TableCell>
                <TableCell sx={adaptiveCellSx(trashRecordWidths.project_name)}>{r.project_name}</TableCell>
                <TableCell sx={adaptiveCellSx(trashRecordWidths.user_name)}>{r.user_name}</TableCell>
                <TableCell align="right" sx={adaptiveCellSx(trashRecordWidths.quantity)}>{r.quantity}</TableCell>
                <TableCell align="right" sx={adaptiveCellSx(trashRecordWidths.actions)}>
                  <Button size="small" color="success" onClick={async () => { try { const res = r.trash_source === 'work' ? await restoreRecord(r.id) : r.trash_source === 'rd' ? await restoreRdRecord(r.id) : await restoreSampleInfo(r.id); if (res.code === 0) { sm('恢复成功'); loadTrash(); } else sm(res.message, true); } catch { sm('恢复失败', true); } }} sx={{ borderRadius: R }}>恢复</Button>
                </TableCell>
              </TableRow>
            ))}</TableBody>
          </Table>
        </TableContainer>)}
      {trashTab === 'types' && (trTypes.length === 0 ? <Typography color="text.secondary" textAlign="center" sx={{ py: 4 }}>回收站为空</Typography>
        : <TableContainer component={Paper} className="table-responsive" sx={tSx}><Table size="small"><TableHead><TableRow>
          <TableCell sx={{ fontWeight: 600 }}>ID</TableCell><TableCell sx={{ fontWeight: 600 }}>类型标识</TableCell><TableCell sx={{ fontWeight: 600 }}>名称</TableCell><TableCell sx={{ fontWeight: 600 }}>创建时间</TableCell><TableCell align="right" sx={{ fontWeight: 600 }}>操作</TableCell>
        </TableRow></TableHead><TableBody>{trTypes.map(t => <TableRow key={t.id} hover><TableCell>{t.id}</TableCell><TableCell>{t.type_key}</TableCell><TableCell>{t.label}</TableCell><TableCell>{t.created_at}</TableCell><TableCell align="right"><Button size="small" color="success" onClick={async () => { try { const res = await updateSampleInfoType(t.id, { is_active: 1 }); if (res.code === 0) { sm('恢复成功'); loadTrash(); } else sm(res.message, true); } catch { sm('恢复失败', true); } }} sx={{ borderRadius: R, mr: 0.5 }}>恢复</Button><Button size="small" color="error" onClick={async () => { if (!window.confirm('确认彻底删除此类型？')) return; try { await deleteSampleInfoTypePermanent(t.id); sm('已彻底删除'); loadTrash(); } catch { sm('删除失败', true); } }} sx={{ borderRadius: R }}>彻底删除</Button></TableCell></TableRow>)}</TableBody></Table></TableContainer>)}
      {trashTab === 'projects' && (trProjects.length === 0 ? <Typography color="text.secondary" textAlign="center" sx={{ py: 4 }}>回收站为空</Typography>
        : <TableContainer component={Paper} className="table-responsive" sx={tSx}><Table size="small"><TableHead><TableRow>
          <TableCell sx={{ fontWeight: 600 }}>ID</TableCell><TableCell sx={{ fontWeight: 600 }}>名称</TableCell><TableCell sx={{ fontWeight: 600 }}>全称</TableCell><TableCell sx={{ fontWeight: 600 }}>创建时间</TableCell><TableCell align="right" sx={{ fontWeight: 600 }}>操作</TableCell>
        </TableRow></TableHead><TableBody>{trProjects.map(p => <TableRow key={p.id} hover><TableCell>{p.id}</TableCell><TableCell>{p.name}</TableCell><TableCell>{p.full_name || '-'}</TableCell><TableCell>{p.created_at}</TableCell><TableCell align="right"><Button size="small" color="success" onClick={async () => { try { const res = await updateProject(p.id, { is_active: true }); if (res.code === 0) { sm('恢复成功'); loadTrash(); } else sm(res.message, true); } catch { sm('恢复失败', true); } }} sx={{ borderRadius: R, mr: 0.5 }}>恢复</Button></TableCell></TableRow>)}</TableBody></Table></TableContainer>)}
      ...
      <Dialog open={Boolean(auditDetail)} onClose={() => setAuditDetail(null)} fullWidth maxWidth="lg">
        ...
        <DialogContent dividers>
          {auditDetail && <>
            ...
            <Typography variant="subtitle2" sx={{ mb: 1 }}>字段快照</Typography>
            <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' }, gap: 1.5, mb: 3 }}>
              <Box><Typography variant="caption" color="text.secondary">修改前</Typography><Box component="pre" sx={{ m: 0, mt: 0.5, p: 1.5, bgcolor: '#f7f7f7', border: '1px solid #e0e0e0', fontSize: 12, lineHeight: 1.55, whiteSpace: 'pre-wrap', wordBreak: 'break-word', maxHeight: 280, overflow: 'auto' }}>{JSON.stringify(auditDetail.before_data ?? {}, null, 2)}</Box></Box>
              <Box><Typography variant="caption" color="text.secondary">修改后</Typography><Box component="pre" sx={{ m: 0, mt: 0.5, p: 1.5, bgcolor: '#f7f7f7', border: '1px solid #e0e0e0', fontSize: 12, lineHeight: 1.55, whiteSpace: 'pre-wrap', wordBreak: 'break-word', maxHeight: 280, overflow: 'auto' }}>{JSON.stringify(auditDetail.after_data ?? {}, null, 2)}</Box></Box>
            </Box>
    ...
    {showSectionContent && tb === 'users' && <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
        ...
        <Button variant="outlined" size="small" startIcon={<AddIcon />} onClick={() => { setImportDialogOpen(true); setImportFile(null); setImportResult(null); }} sx={{ borderRadius: R, borderColor: '#2e7d32', color: '#2e7d32' }}>批量导入</Button>
      </Box>
      ...
      <TableContainer component={Paper} sx={tSx}>
        <Table size="small">
          ...
          <TableBody>
            ...
            {users.length === 0 && (
              <TableRow><TableCell colSpan={9} align="center" sx={{ color: '#999', py: 3 }}>暂无用户</TableCell></TableRow>
            )}
          </TableBody>
        </Table>
      </TableContainer>
          ...
          <Table size="small" stickyHeader>
            ...
            <TableBody>{instruments.map(i => <TableRow key={i.id} hover>
              ...
              <TableCell align="right">
                ...
                <Tooltip title="删除"><IconButton size="small" color="error" onClick={() => { setCa(() => async () => { const r = await deleteInstrument(i.id); if (r.code === 0) { sm('删除成功'); await li(); } else sm(r.message, true); setCo(false); }); setCo(true); }}><DeleteIcon fontSize="small" /></IconButton></Tooltip>
              </TableCell>
            </TableRow>)}</TableBody>
          </Table>
        </TableContainer>
    ...
    <Dialog open={importDialogOpen} onClose={() => setImportDialogOpen(false)} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: R } }}>
      ...
      <DialogContent>
        <Box sx={{ mb: 2 }}>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>① 下载模板</Typography>
          <Button variant="outlined" size="small" onClick={() => {
            const csv = "\uFEFF" + "用户名,密码,真实姓名,所属部门,所属实验室,角色,启用\nzhangsan,123456,张三,研究院,A001,查看者,1\n";
            const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
            const url = URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = url; a.download = 'users_template.csv';
            a.click(); URL.revokeObjectURL(url);
          }} sx={{ borderRadius: R }}>📄 users_template.csv</Button>
        </Box>
        <Box sx={{ mb: 2 }}>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>② 上传文件（CSV 格式，第一行为表头）</Typography>
          <Button variant="outlined" component="label" sx={{ borderRadius: R }}>
            选择文件
            <input type="file" hidden accept=".csv,.xlsx,.xls" onChange={e => { setImportFile(e.target.files?.[0] || null); setImportResult(null); e.target.value = ''; }} />
          </Button>
