...
import { getHelpDocuments, getHelpArticles, getHelpDocumentPageBlob, getHelpDocumentFileBlob } from '../api/client';
...
const HelpPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    getHelpDocumentFileBlob(viewing.data.id)
      .then(blob => {
        if (!active) return;
        objectUrl = URL.createObjectURL(blob);
        setDocumentFileUrl(objectUrl);
      })
      .catch(() => setDocumentFileUrl(null));
    return () => {
      active = false;
      if (objectUrl) URL.revokeObjectURL(objectUrl);
    };
  }, [viewing?.type === 'doc' ? viewing.data.id : null]);

  useEffect(() => {
    if (viewing?.type !== 'doc' || viewing.data.page_count <= 0) {
    ...
    let objectUrl: string | null = null;
    setDocumentPageUrl(null);
    getHelpDocumentPageBlob(viewing.data.id, pageNum)
      .then(blob => {
        if (!active) return;
        objectUrl = URL.createObjectURL(blob);
        setDocumentPageUrl(objectUrl);
        setImgLoadError(false);
      })
      .catch(() => setImgLoadError(true));
    return () => {
      active = false;
      if (objectUrl) URL.revokeObjectURL(objectUrl);
    };
  }, [viewing?.type === 'doc' ? viewing.data.id : null, pageNum]);
        ...
        <Dialog open fullWidth maxWidth="lg" onClose={handleClose} PaperProps={{ sx: { borderRadius: R, height: '90vh' } }}>
          ...
          <DialogContent sx={{ p: 0, flex: 1, overflow: 'auto', display: 'flex', flexDirection: 'column', alignItems: 'center', bgcolor: '#f5f5f5' }}>
            ...
            {viewing.data.file_type === 'pdf' && (viewing.data.page_count > 0 && !imgLoadError) ? (<>
              ...
              <Box sx={{ flex: 1, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', width: '100%' }}>
                <img src={documentPageUrl || undefined} alt={`第 ${pageNum} 页`} style={{ maxWidth: '100%', height: 'auto' }} />
              </Box>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, py: 1.5 }}>
                <IconButton size="small" disabled={pageNum <= 1} onClick={() => setPageNum(p => p - 1)}><ArrowBackIosIcon fontSize="small" /></IconButton>
                <Typography variant="body2" color="text.secondary">{pageNum} / {totalPages}</Typography>
              ...
              <Box sx={{ p: 4, textAlign: 'center', width: '100%', bgcolor: '#fff' }}>
                ...
                <Button component="a" variant="outlined" href={documentFileUrl ?? '#'} download={viewing.data.filename} disabled={!documentFileUrl} sx={{ mt: 2, borderRadius: R }}>下载查看</Button>
              </Box>
            )}
          </DialogContent>
        </Dialog>
