...
const RdRecordsPage: React.FC = () => {
  ...
  const renderStatus = (status: string) => {
    const normalized = status === '已取样' ? '待检测' : status;
    const done = normalized === '已检测';
    const waiting = normalized === '待检测';
    return (
      <Typography variant="body2" sx={{
        ...
        fontSize: '0.75rem',
        fontWeight: 600,
        bgcolor: done ? '#c8e6c9' : waiting ? '#fff9c4' : '#ffcdd2',
        color: done ? '#2e7d32' : waiting ? '#f57f17' : '#c62828',
      }}>
        {normalized}
      </Typography>
    );
  ...
  const getFieldDisplay = (rec: WorkRecord, col: RdRecordColumn) => {
    ...
    const isSampled = status === '已取样' || status === '待检测' || status === '已检测';
    ...
    if (col.name === 'sampling_person') {
      if (status === '已检测') return <Typography variant="body2" sx={{ color: '#2e7d32', fontWeight: 600 }}>{rec.detected_by || rec.sampler || '-'}</Typography>;
      if (isSampled && hasPermission('sample:complete')) return (
        <Button variant="contained" size="small" color="warning" sx={{ borderRadius: R, fontSize: '0.7rem', minWidth: 0, px: 1.25, py: 0 }} onClick={() => handleComplete(rec.id)}>
          完成
        </Button>
      );
      if (isSampled) return <Typography variant="body2" sx={{ color: '#f57f17', fontWeight: 600 }}>{rec.sampler || '待检测'}</Typography>;
