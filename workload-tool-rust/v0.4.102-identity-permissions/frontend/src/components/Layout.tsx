    ...
    <Dialog open={aboutOpen} onClose={() => setAboutOpen(false)} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: '2px' } }}>
      ...
      <DialogContent>
        ...
        <Alert severity="info" sx={{ mb: 2, borderRadius: '2px' }}>
          <Typography variant="subtitle2" sx={{ fontWeight: 600, mb: 0.5 }}>v0.4.102 身份权限与会话安全版</Typography>
          <Typography variant="body2">用户采用永久用户 ID 与用户名联合校验，记录、审计和权限判断统一关联用户身份。</Typography>
          <Typography variant="body2">保留多角色和角色新增功能；五个固定角色权限受保护，管理员仍可维护自定义角色。</Typography>
          <Typography variant="body2">同一用户最多保留两台设备在线，第三次登录自动取消最早会话并立即使旧凭证失效。</Typography>
          <Typography variant="body2">分析检测与研发送样按角色和实验室限定记录、统计、回收站及审计数据范围。</Typography>
          <Typography variant="body2">同名方法按绑定仪器形成独立实例，仪器检测量按仪器 ID 分开统计并显示仪器编号。</Typography>
          <Typography variant="body2">主数据模板按“方法名称 + 仪器编号”绑定，项目关联支持同一项目多行选择实验室和方法。</Typography>
        </Alert>
        <Alert severity="warning" sx={{ display: 'none', mb: 2, borderRadius: '2px' }}>
          <Typography variant="subtitle2" sx={{ fontWeight: 600, mb: 0.5 }}>v0.4.94-trace-preview 管理改版预览版</Typography>
          <Typography variant="body2">• 本版为尝试性预览版本，覆盖安装前请先完成全量备份</Typography>
          <Typography variant="body2">• 管理首页改为主数据、权限与人员、数据治理、系统配置四组侧栏导航</Typography>
          <Typography variant="body2">• 所有管理模块进入统一二级工作区，不再使用三列入口卡片</Typography>
          <Typography variant="body2">• 用户、角色权限和登录会话纳入同一管理导航体系</Typography>
          <Typography variant="body2">• 登录会话支持查看有效期及清理过期记录</Typography>
          <Typography variant="body2">• 手机端使用管理导航抽屉，并在管理路由隐藏全局底部导航避免遮挡</Typography>
          <Typography variant="body2">• 保留 v0.4.93 的导出金额表头、数据溯源和历史工作量口径</Typography>
        </Alert>
