...
fn export_scope(pool: &DbPool, headers: &HeaderMap) -> Result<(Option<i64>, Option<i64>)> {
    ...
    Ok(match ctx.rd_scope()? {
        RecordScope::Global | RecordScope::AnalysisAll => (None, None),
        RecordScope::Lab(group_id) => (None, Some(group_id)),
        RecordScope::Own => (Some(ctx.user.id), None),
    })
}
...
async fn preview_sheet1(
    ...
    let data = rd_export_data::query_sheet1_data(&conn, &start, &end, q.group_id, user_id, group_id)?;
