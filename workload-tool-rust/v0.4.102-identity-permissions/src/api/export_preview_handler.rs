...
use axum::{extract::{Query, State}, http::HeaderMap, Router, response::Json, routing::get};
use chrono::Datelike;
use crate::db::DbPool;
...
fn export_scope(pool: &DbPool, headers: &HeaderMap) -> Result<Option<i64>> {
    ...
    authz_service::require_permission(&ctx, "stats:workload:access")?;
    Ok(if ctx.is_system_admin() || ctx.has_permission("stats:workload:view-all") { None } else { Some(ctx.user.id) })
}
...
async fn preview_sheet1(
    ...
    let conn = pool.get()?;
    let data = export_data::query_sheet1_data(&conn, &start, &end, q.group_id, export_scope(&pool, &headers)?)?;
...
async fn preview_sheet10(
    ...
    let (start, end) = resolve_date_range(&q);
    let conn = pool.get()?;
    let data = export_data::query_sheet10_data(&conn, &start, &end, export_scope(&pool, &headers)?)?;
    Ok(Json(ApiResponse::ok(data)))
}
...
async fn preview_sheet11(
    ...
    let (start, end) = resolve_date_range(&q);
    let data = stats_service::by_division(&pool, &start, &end, None, export_scope(&pool, &headers)?)?;
    Ok(Json(ApiResponse::ok(data)))
}
