...
async fn upload(
    ...
    if ext == "docx" || ext == "pdf" || ext == "xlsx" || ext == "xls" {
        ...
        if ext == "docx" {
            if let Some(result) = crate::api::docx_parser::parse_docx(&file_data)
            ...
            if let Some(result) = crate::api::xlsx_parser::parse_xlsx(&file_data)
                .map_err(|e| tracing::warn!("Excel解析: {}", e)).ok()
            {
                if let Err(e) = crate::repo::article_repo::create(
                    &pool, article_title, &result.html, None, source.as_deref()
                ) {
                    tracing::warn!("保存文章失败: {}", e);
                }
            }
...
struct DocReorderItem { id: i64, sort_order: i64 }
#[derive(serde::Deserialize)]
struct DocReorderBody { ids: Vec<DocReorderItem> }
async fn reorder_docs(
    ...
    help_repo::reorder_documents(&pool, &items)?;
