use axum::{extract::{Multipart, Path, State}, http::HeaderMap, Json, Router};
...
async fn create(State(pool): State<DbPool>, headers: HeaderMap, Json(b): Json<MethodCreate>) -> Result<Json<ApiResponse<MethodResponse>>> {
    let ctx = require_manager(&pool, &headers)?;
    Ok(Json(ApiResponse::ok(method_repo::create(&pool, &b, &ctx.user.username)?)))
}

async fn update(State(pool): State<DbPool>, headers: HeaderMap, Path(id): Path<i64>, Json(b): Json<MethodUpdate>) -> Result<Json<ApiResponse<MethodResponse>>> {
    let ctx = require_manager(&pool, &headers)?;
    Ok(Json(ApiResponse::ok(method_repo::update(&pool, id, &b, &ctx.user.username)?)))
}

async fn delete(State(pool): State<DbPool>, headers: HeaderMap, Path(id): Path<i64>) -> Result<Json<ApiResponse<()>>> {
    let ctx = require_manager(&pool, &headers)?;
    method_repo::delete(&pool, id, &ctx.user.username)?;
...
async fn create_method_type(State(pool): State<DbPool>, headers: HeaderMap, Json(b): Json<MethodTypeCreate>) -> Result<Json<ApiResponse<MethodType>>> {
    let ctx = require_manager(&pool, &headers)?;
    Ok(Json(ApiResponse::ok(method_repo::create_method_type(&pool, &b, &ctx.user.username)?)))
}

async fn update_method_type(State(pool): State<DbPool>, headers: HeaderMap, Path(id): Path<i64>, Json(b): Json<MethodTypeUpdate>) -> Result<Json<ApiResponse<MethodType>>> {
    let ctx = require_manager(&pool, &headers)?;
    Ok(Json(ApiResponse::ok(method_repo::update_method_type(&pool, id, &b, &ctx.user.username)?)))
}

async fn delete_method_type(State(pool): State<DbPool>, headers: HeaderMap, Path(id): Path<i64>) -> Result<Json<ApiResponse<()>>> {
    let ctx = require_manager(&pool, &headers)?;
    method_repo::delete_method_type(&pool, id, &ctx.user.username)?;
    Ok(Json(ApiResponse::ok_msg("删除成功")))
}
...
async fn method_import(State(pool): State<DbPool>, headers: HeaderMap, mut mp: Multipart) -> Result<Json<ApiResponse<ImportSummary>>> {
    require_manager(&pool, &headers)?;
    let _ = (pool, &mut mp);
    return Err(AppError::Validation("旧方法导入已停用，请使用“主数据一键导入”下载新版模板，按方法名称、仪器编号和检测类型导入".into()));
