...
async fn export_excel(
    ...
    let ctx = match authz_service::authenticate(&pool, &headers)
        .and_then(|ctx| {
    ...
    let result: std::result::Result<Vec<u8>, String> = (|| {
        ...
        {
            let div_data = stats_service::by_division(&pool, start, end, None, subject_user_id).map_err(|e| format!("事业部查询: {}", e))?;
    ...
    match result {
            ...
            crate::repo::audit_repo::log_actor(&pool, "export", "work_records", None, ctx.user.id, &ctx.user.username, &desc, "work").ok();
