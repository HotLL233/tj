...
fn scope_user_id(pool: &DbPool, headers: &HeaderMap) -> Result<Option<i64>> {
    ...
    crate::service::authz_service::require_permission(&ctx, "stats:workload:access")?;
    if matches!(
        ctx.workload_scope(),
        crate::service::authz_service::RecordScope::Global
            | crate::service::authz_service::RecordScope::AnalysisAll
    ) {
        Ok(None)
    } else {
        Ok(Some(ctx.user.id))
...
async fn by_division(
    ...
    let scoped_user = scope_user_id(&pool, &headers)?;
    let summaries = crate::service::stats_service::by_division(
        &pool,
        start,
        end,
        q.division_id,
