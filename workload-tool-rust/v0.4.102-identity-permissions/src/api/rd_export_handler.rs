...
async fn export_excel(
    ...
    let result: std::result::Result<Vec<u8>, String> = (|| {
        ...
        // ========== Sheet 1: 各实验室项目方法对应表 ==========
        match rd_export_data::query_sheet1_data(&conn, start, end, q.group_id, subject_user_id, scope_group_id) {
            Ok(data) => {
    ...
    match result {
            ...
            crate::repo::audit_repo::log_actor(&pool, "export", "rd_work_records", None, ctx.user.id, &ctx.user.username, &desc, "rd").ok();
