...
pub fn delete(pool: &DbPool, id: i64, operator: &str) -> Result<()> {
    ...
    audit_repo::log_on_conn_with_module(&tx, "delete", "divisions", Some(id), operator, &format!("删除事业部（解除 {} 个实验室关联）", affected), "shared")?;
    tx.commit()?;
    Ok(())
}
...
pub fn restore(pool: &DbPool, id: i64, operator: &str) -> Result<Division> {
    ...
    audit_repo::log_on_conn_with_module(&tx, "restore", "divisions", Some(id), operator, &format!("恢复事业部「{}」", div.name), "shared")?;
    tx.commit()?;
    Ok(div)
}
