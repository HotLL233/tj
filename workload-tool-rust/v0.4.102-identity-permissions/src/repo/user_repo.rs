...
fn user_perms_multi_on_conn(conn: &rusqlite::Connection, user_id: i64) -> Result<Vec<String>> {
    let mut stmt = conn.prepare(
...
fn user_role_ids_on_conn(conn: &rusqlite::Connection, user_id: i64) -> Result<Vec<i64>> {
    let mut stmt =
...
fn user_role_names_on_conn(conn: &rusqlite::Connection, user_id: i64) -> Result<Vec<String>> {
    let mut stmt = conn.prepare(
...
pub fn create(
    ...
    tx.execute(
        ...
        rusqlite::params![data.username, password_hash, data.division_id, data.group_id, role_id],
    )?;
    ...
    if let Some((actor_id, actor_name)) = actor {
        audit_repo::log_actor_on_conn(
            &tx, "create", "users", Some(id), actor_id, actor_name, &detail, "shared",
        )?;
    } else {
        audit_repo::log_on_conn_with_module(
            &tx, "create", "users", Some(id), &data.username, &detail, "shared",
        )?;
    }
    tx.commit()?;
...
pub fn update(
    ...
    if let Some(rid) = data.role_id {
        tx.execute(
            "UPDATE users SET role_id = ?1, updated_at = datetime('now','localtime') WHERE id = ?2",
    ...
    if changes.is_empty() {
        ...
        audit_repo::log_actor_on_conn(
            &tx, "update", "users", Some(id), actor_id, user_name, &detail, "shared",
        )?;
        tx.commit()?;
    }
...
/// Preserve the stable user ID and audit references while permanently disabling the account.
pub fn permanent_delete(
    ...
    let anonymous_name = format!("deleted_user_{}_{}", id, &uuid::Uuid::new_v4().simple().to_string()[..8]);
    let random_password = bcrypt::hash(uuid::Uuid::new_v4().to_string(), bcrypt::DEFAULT_COST)
        .map_err(|e| AppError::Internal(format!("密码匿名化失败: {e}")))?;

    tx.execute("DELETE FROM user_sessions WHERE user_id=?1", [id])?;
    tx.execute("DELETE FROM user_roles WHERE user_id=?1", [id])?;
    tx.execute(
        "UPDATE users SET username=?1,password=?2,is_admin=0,is_active=0,
                division_id=NULL,group_id=NULL,role_id=NULL,updated_at=datetime('now','localtime')
         WHERE id=?3",
        rusqlite::params![anonymous_name, random_password, id],
    )?;
    audit_repo::log_actor_on_conn(
        &tx,
        "anonymize",
        ...
        actor_id,
        actor_name,
        &format!("匿名化并永久停用用户#{}「{}」", id, existing.username),
        "shared",
    )?;
    tx.commit()?;
    Ok(())
