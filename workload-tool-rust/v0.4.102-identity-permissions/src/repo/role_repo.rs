...
pub fn set_permissions(pool: &DbPool, role_id: i64, body: &RolePermissionSet) -> Result<RoleWithPermissions> {
    ...
    if role.is_system == 1 {
        return Err(AppError::Forbidden("五个固定角色的权限矩阵不能修改".into()));
    }
    for p in &body.permissions {
        if !is_valid_permission(p) {
            return Err(AppError::Validation(format!("非法权限点: {}", p)));
...
mod tests {
    ...
    fn test_pool() -> (DbPool, std::path::PathBuf) {
        let path = std::env::temp_dir().join(format!(
            "role_permissions_{}.db",
            uuid::Uuid::new_v4()
        ));
        let pool = init_pool(path.to_str().unwrap());
    ...
    fn fixed_roles_custom_roles_and_multi_role_union_are_preserved() {
        ...
        assert!(analyst.permissions.iter().any(|value| value == "sample:collect"));
        assert!(analyst.permissions.iter().any(|value| value == "sample:complete"));
        assert!(!analyst.permissions.iter().any(|value| value == "manage:projects"));

        let leader = fixed.iter().find(|role| role.name == "分析检测组长").unwrap();
        assert!(leader.permissions.iter().any(|value| value == "manage:master-import"));
        assert!(leader.permissions.iter().any(|value| value == "manage:users"));
        ...
        let rd_sender_id = fixed.iter().find(|role| role.name == "研发送样员").unwrap().id;
        let user_id = {
            ...
            conn.execute(
                ...
                rusqlite::params![user_id, custom.id],
            ).unwrap();
            conn.execute(
                "INSERT INTO user_roles(user_id,role_id) VALUES(?1,?2)",
                rusqlite::params![user_id, rd_sender_id],
            ).unwrap();
            user_id
        };
        ...
        migrations::run(&pool.get().unwrap()).unwrap();
        assert!(list_with_permissions(&pool).unwrap().iter().any(|role| {
            role.name == "CustomRole01" && role.is_system == 0
        }));
