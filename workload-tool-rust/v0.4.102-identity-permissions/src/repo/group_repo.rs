...
pub fn create(pool: &DbPool, body: &GroupCreate, operator: &str) -> Result<GroupResponse> {
    ...
    if name.is_empty() { return Err(AppError::Validation("实验室名称不能为空".into())); }
    let duplicate: i64 = conn.query_row("SELECT COUNT(*) FROM project_groups WHERE name=?1", [name], |row| row.get(0))?;
    if duplicate > 0 { return Err(AppError::Validation(format!("实验室名称已存在：{name}"))); }
    conn.execute(
    ...
    audit_repo::log(pool, "create", "project_groups", Some(id), operator, &format!("创建实验室「{}」", body.name))?;
    get_by_id(pool, id)
}
...
pub fn update(pool: &DbPool, id: i64, body: &GroupUpdate, operator: &str) -> Result<GroupResponse> {
    ...
    if let Some(ref name) = body.name {
        let name = name.trim();
        if name.is_empty() { return Err(AppError::Validation("实验室名称不能为空".into())); }
        let duplicate: i64 = conn.query_row("SELECT COUNT(*) FROM project_groups WHERE name=?1 AND id<>?2", rusqlite::params![name, id], |row| row.get(0))?;
        if duplicate > 0 { return Err(AppError::Validation(format!("实验室名称已存在：{name}"))); }
    ...
    audit_repo::log(pool, "update", "project_groups", Some(id), operator, &format!("编辑实验室「{}」", body.name.as_deref().unwrap_or("")))?;
    get_by_id(pool, id)
}
...
pub fn delete(pool: &DbPool, id: i64, operator: &str) -> Result<()> {
    ...
    audit_repo::log(pool, "delete", "project_groups", Some(id), operator, &format!("删除实验室「{}」", g.name))?;
    Ok(())
}
