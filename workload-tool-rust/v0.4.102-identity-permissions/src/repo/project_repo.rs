...
const PROJ_SQL: &str =
    "SELECT p.id, p.name, COALESCE(p.notes,''), \
     ...
     COALESCE(GROUP_CONCAT(DISTINCT m.name || ' · ' || COALESCE(NULLIF(i.code,''),'待配置')), '') as method_names_str, \
     p.created_at, \
     p.high_item, \
...
fn row_to_project(row: &rusqlite::Row) -> rusqlite::Result<ProjectResponse> {
    ...
    Ok(ProjectResponse {
        ...
        project_status: row.get::<_, Option<String>>(12)?.unwrap_or_else(|| "ongoing".to_string()),
...
pub fn list(pool: &DbPool, group_id: Option<i64>, active_only: bool, method_type: Option<&str>, status: Option<&str>) -> Result<Vec<ProjectResponse>> {
    let conn = pool.get()?;
    ...
    sql.push_str(" GROUP BY p.id ORDER BY p.sort_order ASC, p.id ASC");
...
pub fn create(pool: &DbPool, body: &ProjectCreate, user_name: &str) -> Result<ProjectResponse> {
    ...
    if name.is_empty() { return Err(AppError::Validation("项目名称不能为空".into())); }
    let duplicate: i64 = conn.query_row("SELECT COUNT(*) FROM projects WHERE name=?1", [name], |row| row.get(0))?;
    if duplicate > 0 { return Err(AppError::Validation(format!("项目名称已存在：{name}"))); }
    ...
    conn.execute(
        ...
        rusqlite::params![group_id, name, fnm, nt, so, ia, body.high_item, status, user_name],
    )?;
    let pid = conn.last_insert_rowid();
    audit_repo::log(pool, "create", "projects", Some(pid), user_name, &format!("创建项目「{}」", body.name))?;
...
pub fn update(pool: &DbPool, id: i64, body: &ProjectUpdate, user_name: &str) -> Result<ProjectResponse> {
    let conn = pool.get()?;
    if let Some(ref n) = body.name {
        let name = n.trim();
        if name.is_empty() { return Err(AppError::Validation("项目名称不能为空".into())); }
        let duplicate: i64 = conn.query_row("SELECT COUNT(*) FROM projects WHERE name=?1 AND id<>?2", rusqlite::params![name, id], |row| row.get(0))?;
        if duplicate > 0 { return Err(AppError::Validation(format!("项目名称已存在：{name}"))); }
    ...
    if let Some(ref s) = body.project_status {
        ...
        conn.execute(
            "UPDATE projects
             ...
             WHERE id=?3",
            rusqlite::params![status, user_name, id],
        )?;
    }
    if let Some(ref hi) = body.high_item {
        let hi_trim = hi.trim().to_string();
        let val = if hi_trim.is_empty() { None } else { Some(hi_trim) };
        conn.execute("UPDATE projects SET high_item=?1 WHERE id=?2", rusqlite::params![val, id])?;
    }
    // Replace lab links
    if let Some(ref lab_ids) = body.lab_ids {
    ...
    let action = if body.project_status.as_deref() == Some("archived") { "归档项目" } else if body.project_status.as_deref() == Some("ongoing") { "重新启用项目" } else { "编辑项目" };
    audit_repo::log(pool, "update", "projects", Some(id), user_name, &format!("{}「{}」", action, body.name.as_deref().unwrap_or("")))?;
    get_by_id(pool, id)
}
...
pub fn delete(pool: &DbPool, id: i64, operator: &str) -> Result<()> {
    ...
    audit_repo::log(pool, "delete", "projects", Some(id), operator, &format!("删除项目「{}」", proj.name))?;
...
mod tests {
    ...
    fn archive_hides_project_and_reenable_restores_it() {
        let path = std::env::temp_dir().join(format!("project_archive_{}.db", uuid::Uuid::new_v4()));
        let pool = crate::db::init_pool(path.to_str().unwrap());
        {
            let conn = pool.get().unwrap();
            crate::db::migrations::run(&conn).unwrap();
            conn.execute("INSERT INTO project_groups(name,sort_order) VALUES ('实验室A',1)", []).unwrap();
        }
        let group_id = pool.get().unwrap().query_row("SELECT id FROM project_groups WHERE name='实验室A'", [], |row| row.get::<_, i64>(0)).unwrap();
        let created = create(&pool, &ProjectCreate {
            name: "项目A".into(), full_name: None, notes: None, sort_order: None,
            is_active: Some(true), lab_ids: Some(vec![group_id]), method_ids: Some(vec![]),
            high_item: None, project_status: Some("ongoing".into()),
        }, "tester").unwrap();
        assert_eq!(list(&pool, None, true, None, None).unwrap().len(), 1);

        update(&pool, created.id, &ProjectUpdate {
            name: None, full_name: None, notes: None, sort_order: None, is_active: None,
            lab_ids: None, method_ids: None, high_item: None, project_status: Some("archived".into()),
        }, "archiver").unwrap();
        ...
        update(&pool, created.id, &ProjectUpdate {
            name: None, full_name: None, notes: None, sort_order: None, is_active: None,
            lab_ids: None, method_ids: None, high_item: None, project_status: Some("ongoing".into()),
        }, "archiver").unwrap();
        assert_eq!(list(&pool, None, true, None, None).unwrap().len(), 1);
        drop(pool);
