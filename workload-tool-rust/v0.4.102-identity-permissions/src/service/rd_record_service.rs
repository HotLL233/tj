...
pub fn create_record(pool: &DbPool, input: &RecordCreate, batch_no: Option<String>, notes: Option<String>, operator: &str) -> Result<RdRecordResponse, AppError> {
    if input.quantity <= 0 {
...
pub fn complete(pool: &DbPool, id: i64, detector: &str) -> Result<RdRecordResponse, AppError> {
    repo::rd_record_repo::complete(pool, id, detector)
}
