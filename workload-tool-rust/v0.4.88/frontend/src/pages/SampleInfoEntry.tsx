...
const SampleInfoEntry: React.FC = () => {
  ...
  const listRecordWidths = useMemo(() => getAdaptiveColumnWidths(records, [
    ...
    { key: 'seq_no', header: '序号', fixed: tableConfig.seq_column_width || 54, getValue: r => r.seq_no },
  ...
  const formColumnCellSx = (key: string) => adaptiveCellSx(formInputWidths[key] || 100);
  const listColumnCellSx = (key: string) => adaptiveCellSx(listRecordWidths[key] || 90);
      ...
      <Paper elevation={0} sx={{ p: 3, mb: 3, borderRadius: R, border: '2px solid #2e7d32', background: 'linear-gradient(145deg,#ffffff,#f1f8e9)' }}>
        ...
        <TableContainer component={Paper} sx={{ mb: 2, borderRadius: R, border: '1px solid rgba(0,0,0,0.08)', overflowX: 'auto' }}>
          <Table size="small" stickyHeader sx={adaptiveTableSx}>
            <TableHead>
              <TableRow>
                <TableCell sx={{ ...formColumnCellSx('checked'), fontWeight: 700, p: 1 }}>
                  <Checkbox
                    size="small"
                ...
                <TableCell sx={{ ...formColumnCellSx('seq'), fontWeight: 700, p: 1 }}>序号</TableCell>
                {formColumns.map(col => (
                  <TableCell key={col.field_key} sx={{ ...formColumnCellSx(col.field_key), fontWeight: 700, p: 1 }}>
            ...
            <TableBody>
              {rows.map((row, idx) => (
                <TableRow key={idx} hover selected={row.checked}>
                  <TableCell sx={{ ...formColumnCellSx('checked'), p: 0.5 }}>
                    <Checkbox
                      size="small"
                      checked={row.checked}
                  ...
                  <TableCell sx={{ ...formColumnCellSx('seq'), p: 0.5, fontSize: '0.8rem', color: '#999' }}>{idx + 1}</TableCell>
                  {formColumns.map(col => (
                    <TableCell key={col.field_key} sx={{ ...formColumnCellSx(col.field_key), p: 0.5 }}>
                    ...
                    return (
                      <React.Fragment key={r.id}>
                        ...
                        >
                          ...
                          <TableCell sx={{ ...listColumnCellSx('seq_no'), fontSize: '0.875rem', color: '#999', borderColor: '#e0e0e0' }}>#{r.seq_no}</TableCell>
                          {listColumns.filter(c => !['status', 'seq_no'].includes(c.field_key)).map(col => (
