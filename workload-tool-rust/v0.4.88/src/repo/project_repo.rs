...
pub fn create(pool: &DbPool, body: &ProjectCreate) -> Result<ProjectResponse> {
    ...
    conn.execute(
        "INSERT INTO projects (group_id, name, full_name, notes, sort_order, is_active, high_item, project_status, archived_at, archived_by) VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, ?8, CASE WHEN ?8='archived' THEN datetime('now','localtime') ELSE NULL END, NULL)",
        rusqlite::params![group_id, body.name, fnm, nt, so, ia, body.high_item, status],
    )?;
        ...
        conn.execute(
            "UPDATE projects
             SET project_status=?1,
                 ...
                 archived_by=CASE WHEN ?1='archived' THEN COALESCE(archived_by, 'system') ELSE NULL END
             WHERE id=?2",
