...
async fn download_template() -> Result<impl IntoResponse> {
    let bytes = build_simplified_template()?;
    let response = Response::builder()
        ...
        .header(
            ...
            "attachment; filename*=UTF-8''%E4%B8%BB%E6%95%B0%E6%8D%AE%E4%B8%80%E9%94%AE%E5%AF%BC%E5%85%A5%E6%A8%A1%E6%9D%BF_v0.4.77.xlsx",
        )
...
fn build_simplified_template() -> std::result::Result<Vec<u8>, XlsxError> {
    ...
    let wrap = Format::new().set_text_wrap().set_align(FormatAlign::Top);
    {
        ...
        ws.merge_range(0, 0, 0, 1, "主数据一键导入模板 v0.4.88", &title)?;
        let rows = [
            ("主数据", "对象类型填写：部门、实验室、检测类型、检测方法、研发项目。不同对象只填写对应字段。"),
            ("项目关联", "一行一个项目；关联实验室一列、关联方法一列。多个值可用分号、顿号、逗号或换行分隔。"),
            ("元数据", "仅作为填写参考，不参与导入。"),
            ("项目状态", "保留字段；项目归档状态可在项目管理页维护。"),
        ];
        for (idx, (name, desc)) in rows.iter().enumerate() {
            ws.write((idx + 2) as u32, 0, *name)?;
    ...
    add_data_sheet(&mut workbook, "元数据", &["分类", "可填写内容", "说明"], &[18.0, 50.0, 60.0], &[])?;
