/// 导出数据查询层 - v0.3.18 重写版本
/// 支持 10 个 Sheet 的数据查询
/// v0.3.18 修复：导出 Excel 实验室拼接问题 → 分开统计（每个实验室一行）
...
pub fn query_sheet1_data(
    ...
    // v0.3.18 修复：用 LEFT JOIN 替代 group_concat 子查询
    // 按 (实验室, 项目, 方法) 分组，每个实验室独立一行
    ...
    if let Some(gid) = group_id {
        sql.push_str(&format!(" AND pg.id = {}", gid));
    }

    sql.push_str(" GROUP BY pg.name, p.id, m.id ORDER BY lab_name, p.name, m.name");
    ...
    let sql =
        "SELECT date(wr.recorded_at) AS record_date,
         ...
         WHERE wr.deleted_at IS NULL
           AND wr.recorded_at >= ?1
           AND wr.recorded_at <= ?2
         GROUP BY record_date, pg.name, m.id, p.id
    ...
    let sql =
        "SELECT COALESCE(pg.name, '未知') AS lab_name,
         ...
         WHERE wr.deleted_at IS NULL
           AND wr.recorded_at >= ?1
           AND wr.recorded_at <= ?2
         GROUP BY pg.name, p.id, mt.name, m.amount
