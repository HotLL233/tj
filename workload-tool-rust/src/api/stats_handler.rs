...
async fn summary(State(pool): State<DbPool>, Query(q): Query<StatsQuery>) -> Result<Json<ApiResponse<StatsSummary>>> {
    ...
    let (tq, tr, uc, pc): (i64, i64, i64, i64) = conn.query_row(
        ...
        rusqlite::params_from_iter(param_refs.iter()), |r| Ok((r.get(0)?, r.get(1)?, r.get(2)?, r.get(3)?)),
    )?;
    ...
    Ok(Json(ApiResponse::ok(StatsSummary { total_quantity: tq, total_records: tr, user_count: uc, project_count: pc, breakdown })))
}
...
async fn by_instrument(State(pool): State<DbPool>, Query(q): Query<StatsQuery>) -> Result<Json<ApiResponse<Vec<InstrumentStats>>>> {
    ...
    let refs: Vec<&dyn rusqlite::types::ToSql> = params.iter().map(|s| s as &dyn rusqlite::types::ToSql).collect();
    let mut stmt = conn.prepare(&format!(
        "SELECT SUBSTR(p.name, INSTR(p.name,'-')+1) AS instrument, SUM(wr.quantity), COUNT(*), COUNT(DISTINCT wr.user_name) FROM work_records wr JOIN projects p ON wr.project_id=p.id JOIN project_groups pg ON p.group_id=pg.id WHERE p.name LIKE '%LC-%' OR p.name LIKE '%GC-%' AND {} GROUP BY instrument ORDER BY instrument", wc
    ))?;
    let rows = stmt.query_map(rusqlite::params_from_iter(refs.iter()), |row| Ok(InstrumentStats {
        instrument: row.get(0)?, total_quantity: row.get(1)?, record_count: row.get(2)?, user_count: row.get(3)?,
    }))?;
    Ok(Json(ApiResponse::ok(rows.collect::<std::result::Result<Vec<_>, _>>()?)))
}
