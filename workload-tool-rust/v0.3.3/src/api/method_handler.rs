...
async fn method_import(State(pool): State<DbPool>, mut mp: Multipart) -> Result<Json<ApiResponse<ImportSummary>>> {
    ...
    let mut project_lab_pairs: Vec<(String, String)> = Vec::new();
    ...
    // v0.3.0: 构建 project_lab_pairs（同行项目和实验室列的配对）
    for row_idx in 1..rows.len() {
        let mut row_projects: Vec<String> = Vec::new();
        let mut row_groups: Vec<String> = Vec::new();
        for col_idx in 0..ncols {
            let header = &headers[col_idx];
            if let Some(m) = mappings.iter().find(|m| wildcard_match(&m.header_pattern, header)) {
                if let Some(v) = rows[row_idx].get(col_idx) {
                    let val = v.as_string().unwrap_or_default().trim().to_string();
                    if !val.is_empty() {
                        match m.target_table.as_str() {
                            "projects" => row_projects.push(val),
                            "project_groups" => row_groups.push(val),
                            _ => {}
                        }
                    }
                }
        ...
        for p in &row_projects {
            for g in &row_groups {
                project_lab_pairs.push((p.clone(), g.clone()));
            }
        }
    }
    ...
    let summary = method_repo::batch_import_column_split(
        &conn, &group_items, &project_items, &method_items, &project_lab_pairs
    )?;
