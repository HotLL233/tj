...
fn row_to_method(row: &rusqlite::Row) -> rusqlite::Result<MethodResponse> {
    let type_ids_str: String = row.get::<_, String>(7).unwrap_or_default();
    let type_names_str: String = row.get::<_, String>(8).unwrap_or_default();
    Ok(MethodResponse {
        id: row.get(0)?,
        name: row.get(1)?,
        full_name: row.get(2)?,
...
pub fn create(pool: &DbPool, body: &MethodCreate) -> Result<MethodResponse> {
    ...
    conn.execute(
        "INSERT INTO methods (name, full_name, coefficient, amount, notes) VALUES (?1,?2,?3,?4,?5)",
        rusqlite::params![body.name, fn_, cf, amt, nt],
    )?;
...
/// v0.3.0: 新增 project_lab_pairs 支持项目-实验室 M:N 自动关联
pub fn batch_import_column_split(
    conn: &Connection,
    group_names: &[String],
    project_names: &[String],
    method_items: &[(String, String, String)],
    project_lab_pairs: &[(String, String)],
    ...
    for pname in project_names {
        ...
        if existing.is_none() {
            ...
            // v0.3.3: 如果 project_lab_pairs 中有此项目的实验室关联，更新 group_id 为实际实验室
            if let Some((_, lab_name)) = project_lab_pairs.iter().find(|(p, _)| *p == *pname) {
                if let Ok(lab_gid) = conn.query_row(
                    "SELECT id FROM project_groups WHERE name=?1",
                    rusqlite::params![lab_name], |r| r.get::<_, i64>(0),
                ) {
                    conn.execute(
                        "UPDATE projects SET group_id=?1 WHERE id=?2",
                        rusqlite::params![lab_gid, pid],
                    )?;
                }
            }
        }
    ...
    // v0.3.0: 项目-实验室 M:N 自动关联
    for (pname, gname) in project_lab_pairs {
        if let (Ok(pid), Ok(gid)) = (
            conn.query_row("SELECT id FROM projects WHERE name=?1", rusqlite::params![pname], |r| r.get::<_, i64>(0)),
            conn.query_row("SELECT id FROM project_groups WHERE name=?1", rusqlite::params![gname], |r| r.get::<_, i64>(0)),
        ) {
    ...
    for (_group_name, item_name, method_type) in method_items {
