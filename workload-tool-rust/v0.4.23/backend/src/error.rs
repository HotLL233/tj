...
pub enum AppError {
    ...
    #[error("未授权: {0}")]
    Unauthorized(String),                         // code 401
    #[error("禁止访问: {0}")]
    Forbidden(String),                             // code 403
    #[error("IO错误: {0}")]
    Io(#[from] std::io::Error),                    // code 5000
    #[error("图像错误: {0}")]
    Image(#[from] image::ImageError),             // code 5000
}
...
impl AppError {
    pub fn code(&self) -> i32 {
        match self {
            ...
            AppError::Unauthorized(_) => 401,                           // 未登录/凭证无效
            AppError::Forbidden(_) => 403,                              // 权限不足
            AppError::Io(_) | AppError::Image(_) => 5000,              // 内部/IO/图像错误
        }
    }
}
...
impl IntoResponse for AppError {
    fn into_response(self) -> Response {
        ...
        let message = match &self {
            ...
            AppError::Image(e) => format!("图像错误: {}", e),
        };
