...
/// 模块感知审计 — 追加 module 列（保留兼容，新代码请使用 log_with_module_on_conn）
...
/// v0.4.21: 模块感知审计（带字段级 before/after diff），在已有连接上写入（推荐）。
/// 全仓统一通过此函数写审计，不再直接 INSERT audit_log；WORM 触发器禁止对 audit_log 的 UPDATE/DELETE。
pub fn log_with_module_on_conn(
    ...
    conn.execute(
        "INSERT INTO audit_log (action, table_name, record_id, user_name, detail, module, before_json, after_json, created_at) \
         VALUES (?1,?2,?3,?4,?5,?6,?7,?8, datetime('now','localtime'))",
        rusqlite::params!(action, table, record_id, user_name, detail, module, before, after),
    )?;
    Ok(())
}
...
/// - rd  → 研发送样录入 + 共享主数据（module IN ('rd','shared')）—— 不含 work
/// - shared → 仅主数据
/// - 具体模块名 → 仅该模块
/// - None → 全部
pub fn list(pool: &DbPool, page: i64, page_size: i64, module: Option<&str>) -> Result<(Vec<AuditLogResponse>, i64)> {
    ...
    let base = "SELECT id, action, table_name, record_id, user_name, detail, module, before_json, after_json, created_at FROM audit_log";
    ...
    let rows = stmt.query_map([page_size, offset], |row| {
        Ok(AuditLogResponse {
            ...
            before_json: row.get(7)?, after_json: row.get(8)?,
