...
pub fn create(pool: &DbPool, body: &RecordCreate) -> Result<RecordResponse> {
    ...
    audit_repo::log_with_module_on_conn(&tx, "create", "work_records", Some(id), &body.user_name, &detail, "work", None, None)?;
    tx.commit()?;
    get_by_id_on_conn(&conn, id)
}
...
pub fn update(pool: &DbPool, id: i64, body: &RecordUpdate, user_name: &str) -> Result<RecordResponse> {
    ...
    audit_repo::log_with_module_on_conn(&tx, "update", "work_records", Some(id), user_name, &detail, "work", None, None)?;
    tx.commit()?;
    get_by_id_on_conn(&conn, id)
}
...
pub fn soft_delete(pool: &DbPool, id: i64, user_name: &str) -> Result<()> {
    ...
    audit_repo::log_with_module_on_conn(&tx, "delete", "work_records", Some(id), user_name, &detail, "work", None, None)?;
    tx.commit()?;
    Ok(())
}
...
pub fn restore(pool: &DbPool, id: i64, user_name: &str) -> Result<RecordResponse> {
    ...
    audit_repo::log_with_module_on_conn(&tx, "restore", "work_records", Some(id), user_name, &detail, "work", None, None)?;
    tx.commit()?;
    get_by_id_on_conn(&conn, id)
}
...
pub fn delete_by_user(pool: &DbPool, user_name: &str, start: Option<&str>, end: Option<&str>) -> Result<i64> {
    ...
    audit_repo::log_with_module_on_conn(&tx, "batch_delete", "work_records", None, user_name, &format!("批量删除 {} 条", count), "work", None, None)?;
    tx.commit()?;
    Ok(count as i64)
}
