...
pub fn create(pool: &DbPool, body: &RecordCreate) -> Result<RdRecordResponse> {
    ...
    audit_repo::log_with_module_on_conn(&tx, "create", "rd_work_records", Some(id), &body.user_name, &detail, "rd", None, None)?;
    tx.commit()?;
    get_by_id_on_conn(&conn, id)
}
...
pub fn update(pool: &DbPool, id: i64, body: &RecordUpdate, user_name: &str) -> Result<RdRecordResponse> {
    ...
    audit_repo::log_with_module_on_conn(&tx, "update", "rd_work_records", Some(id), user_name, &detail, "rd", None, None)?;
    tx.commit()?;
    get_by_id_on_conn(&conn, id)
}
...
pub fn soft_delete(pool: &DbPool, id: i64, user_name: &str) -> Result<()> {
    ...
    audit_repo::log_with_module_on_conn(&tx, "delete", "rd_work_records", Some(id), user_name, &detail, "rd", None, None)?;
    tx.commit()?;
    Ok(())
}
...
pub fn restore(pool: &DbPool, id: i64, user_name: &str) -> Result<RdRecordResponse> {
    ...
    audit_repo::log_with_module_on_conn(&tx, "restore", "rd_work_records", Some(id), user_name, &detail, "rd", None, None)?;
    tx.commit()?;
    get_by_id_on_conn(&conn, id)
}
...
pub fn delete_by_user(pool: &DbPool, user_name: &str, start: Option<&str>, end: Option<&str>) -> Result<i64> {
    ...
    audit_repo::log_with_module_on_conn(&tx, "batch_delete", "rd_work_records", None, user_name, &format!("批量删除 {} 条", count), "rd", None, None)?;
    tx.commit()?;
    Ok(count as i64)
}
