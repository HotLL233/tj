...
const getRecordFieldWidth = (key: string, fallback?: number) => {
  ...
  return fixed[key] || Math.max(fallback || 100, 90);
};
  ...
  const pageContent = (
    ...
    <Box sx={{ p: 2 }}>
      ...
      <Paper elevation={0} sx={{ p: 1.5, mb: 2, borderRadius: R, border: '2px solid #2e7d32', background: 'linear-gradient(145deg,#ffffff,#f1f8e9)' }}>
        ...
        {rows.length > 0 && (
        <TableContainer component={Paper} variant="outlined" sx={{ borderRadius: R, boxShadow: 'none', mb: 1.25, overflowX: 'auto' }}>
      ...
      <Box sx={{ mt: 3 }}>
        ...
        {recordsLoading && todayRecords.length === 0 ? (
          ...
          <TableContainer component={Paper} variant="outlined" sx={{ borderRadius: R, boxShadow: 'none', overflowX: 'auto' }}>
            <Table size="small" sx={{ width: 'max-content', minWidth: Math.max(recordTableMinWidth, 1120), tableLayout: 'fixed' }}>
              <TableHead>
                <TableRow sx={{ bgcolor: 'rgba(230,81,0,0.06)' }}>
                  <TableCell sx={{ fontWeight: 700, fontSize: '0.8rem', whiteSpace: 'nowrap', width: tableConfig.seq_column_width, textAlign: 'center' }}>序号</TableCell>
