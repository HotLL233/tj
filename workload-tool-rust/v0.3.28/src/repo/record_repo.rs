...
pub fn create(pool: &DbPool, body: &RecordCreate) -> Result<RecordResponse> {
    ...
    tx.execute(
        "INSERT INTO work_records (project_id, method_id, user_name, quantity, recorded_at, group_id)
         VALUES (?1, ?2, ?3, ?4, ?5, ?6)",
...
pub fn update(pool: &DbPool, id: i64, body: &RecordUpdate, user_name: &str) -> Result<RecordResponse> {
    ...
    let detail = format!("修改记录#{}：项目「{}」", id, existing.project_name);
...
pub fn soft_delete(pool: &DbPool, id: i64, user_name: &str) -> Result<()> {
    ...
    let detail = format!("删除记录#{}：项目「{}」/ 方法「{}」，数量 {}", id, rec.project_name, rec.method_name, rec.quantity);
...
pub fn restore(pool: &DbPool, id: i64, user_name: &str) -> Result<RecordResponse> {
    ...
    let detail = format!("恢复记录#{}：项目「{}」/ 方法「{}」，数量 {}", id, r.project_name, r.method_name, r.quantity);
