...
pub fn query_sheet1_data(
    ...
    let mut sql = String::from(
        "SELECT COALESCE(pg.name, '未分组') as lab_name, p.name, m.full_name, m.name, m.coefficient,
    ...
    if let Some(gid) = group_id {
        sql.push_str(" AND pll.group_id = ?3");
        params.push(Box::new(gid));
    }

    sql.push_str(" GROUP BY pg.id, p.id, m.id ORDER BY COALESCE(pg.sort_order, 999), p.name, m.name");
...
pub fn query_sheet2_data(
    ...
    let sql =
        "SELECT date(wr.recorded_at) AS record_date,
                m.full_name,
                COALESCE(pg.name, '未分组') AS lab_name,
         ...
         WHERE wr.deleted_at IS NULL
           AND wr.recorded_at >= ?1
           AND wr.recorded_at <= ?2
         GROUP BY record_date, m.id, p.id, pg.id
         ORDER BY record_date, m.full_name, COALESCE(pg.sort_order, 999), p.name";
    ...
    let sql =
        "SELECT p.name AS project_name,
         ...
         WHERE wr.deleted_at IS NULL
           AND wr.recorded_at >= ?1
           AND wr.recorded_at <= ?2
         GROUP BY p.id, m.id, pg.id
         ORDER BY p.name, COALESCE(pg.sort_order, 999), m.name";
...
pub fn query_sheet4_data(
    ...
    let sql =
        "SELECT COALESCE(pg.name, '未分组') AS lab_name,
                p.name AS project_name,
                m.full_name,
                m.name AS method_name,
                m.amount,
         ...
         ORDER BY COALESCE(pg.sort_order, 999), p.name, m.name";
...
pub fn query_sheet7_data(
    ...
    let sql =
        "SELECT COALESCE(pg.name, '未分组') AS lab_name,
         ...
         ORDER BY COALESCE(pg.sort_order, 999), p.name, mt.name";
